...
class UsuariosRolesTest extends TestCase
{
    ...
    public function test_un_rol_nuevo_entra_solo_a_lo_que_tiene_marcado(): void
    {
        ...
        $this->actingAs($encargado)->get('/admin/configuracion/tienda')->assertForbidden();
    ...
    public function test_el_administrador_entra_a_todo_aunque_le_borren_los_permisos(): void
    {
        ...
        $this->actingAs($this->admin())->get('/admin/configuracion/tienda')->assertOk();
    }
    ...
    {
        User::factory()->create(['rol' => 'vendedor', 'name' => 'Ayelen Vargas']);
    ...
    {
        User::factory()->create(['rol' => 'vendedor', 'name' => 'Ayelen Vargas', 'email' => 'Ayelen@Correo.com']);

        $this->actingAs($this->admin())->get('/admin/usuarios?q=VARGAS')
            ->assertInertia(fn (Assert $page) => $page->has('usuarios', 1));

        $this->actingAs($this->admin())->get('/admin/usuarios?q=ayelen@correo')
            ->assertInertia(fn (Assert $page) => $page->has('usuarios', 1));
    }
    ...
    public function test_crear_un_usuario_con_su_rol_y_su_contrasena(): void
    {
        ...
        $u = User::where('email', 'jhoel@correo.com')->first();
        $this->assertNotNull($u);
        $this->assertSame('Jhoel Abasto', $u->name);
    ...
    {
        $otro = User::factory()->create(['rol' => 'vendedor', 'name' => 'Ayelen']);

        $this->actingAs($this->admin())->patch("/admin/usuarios/{$otro->id}", [
            'name' => 'Ayelen', 'email' => $otro->email, 'rol' => 'vendedor',
    ...
    public function test_el_catalogo_de_modulos_cubre_las_rutas_del_panel(): void
    {
        ...
        $this->assertSame('tienda', Permisos::moduloDeRuta('admin.configuracion.tienda.edit'));
        $this->assertSame('marketing', Permisos::moduloDeRuta('admin.seo.index'));
