...
class ExportacionesTest extends TestCase
{
    ...
    public function test_el_pdf_lleva_los_datos_de_la_tienda_y_no_los_del_codigo(): void
    {
        \App\Models\StoreLocation::create([
            'name' => 'Casa matriz', 'address' => 'Av. Heroínas 456', 'city' => 'Cochabamba',
            'country' => 'Bolivia', 'phone' => '59144123456', 'active' => true, 'sort_order' => 1,
        ]);
        \App\Models\ConfiguracionTienda::set('tienda_nombre', 'Mac Center BO');
        \App\Models\ConfiguracionTienda::clearAllCache();

        $this->actingAs($this->admin())->get('/admin/exportar')
            ->assertInertia(fn (Assert $page) => $page
                ->where('tienda.nombre', 'Mac Center BO')
