 ...
 * permutas y reservas, publicación en la tienda, protección al eliminar y condición en lote.
 *
...
trait EquipoDeInventario
{
    ...
    /** Equipos con publicación en la tienda: id → publicado (true) o borrador (false). */
    protected function publicacionesEnTienda(): array
    {
        return CatalogoPublicacion::where('producto_tipo', $this->inventario()['tipo'])
            ->get(['producto_id', 'publicado'])
            ->mapWithKeys(fn ($p) => [(int) $p->producto_id => (bool) $p->publicado])
            ->all();
    }
    ...
    protected function motivoBloqueo(Model $equipo): ?string
    {
        ...
        if (CatalogoPublicacion::where('producto_tipo', $tipo)->where('producto_id', $id)->exists()) {
            return 'Está en Productos en la tienda: primero elimina su publicación.';
        }

        return null;
    }

    protected function publicacionDe(Model $equipo): ?array
    {
        $pub = CatalogoPublicacion::where('producto_tipo', $this->inventario()['tipo'])
            ->where('producto_id', $equipo->getKey())
            ->first();
        if (! $pub) {
            return null;
        }

        $estado = $pub->estadoPublicacion();

        return [
            'titulo' => $pub->titulo,
            'estado' => $estado,
            'editar' => route('admin.catalogo.edit', $pub),
            'ver'    => $estado === 'Publicado' ? route('store.product', $pub->slug) : null,
        ];
    }
    ...
    /**
     * Marca varios equipos como Nuevo o Seminuevo de una vez; sus publicaciones se actualizan solas.
