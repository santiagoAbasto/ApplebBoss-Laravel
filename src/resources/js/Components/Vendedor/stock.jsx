...
function Celda({ item, col }) {
    ...
    if (col.precio) return <span className="font-bold tabular-nums text-slate-900">{bsFmt(item[col.key])}</span>;
                    ...
                    <li key={item.id} className="px-4 py-4">
                        <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                                <p className="truncate text-[15px] font-bold text-slate-900">{txt(item[principal.key]) || 'Producto'}</p>
                                <div className="mt-1"><CondicionBadge condicion={item.condicion} vacio={null} /></div>
                            </div>
                            {precio && <p className="shrink-0 text-[15px] font-extrabold tabular-nums text-slate-900">{bsFmt(item[precio.key])}</p>}
                        </div>

                        <dl className="mt-3 grid grid-cols-2 gap-2">
                            {resto.filter((c) => !c.condicion && txt(item[c.key])).map((c) => (
                                <div key={c.key} className="min-w-0 rounded-lg bg-slate-50 px-2.5 py-1.5">
                                    <dt className="text-[10px] font-bold uppercase tracking-wide text-slate-400">{c.label}</dt>
                                    <dd className={`truncate text-[13px] font-semibold text-slate-700 ${c.mono ? 'font-mono text-[11px]' : ''}`}>
                ...
                <table className="w-full text-left text-[13px]">
                    <thead>
                        <tr className="border-b border-slate-200 bg-slate-50">
                            {cols.map((c) => (
                                <th key={c.key} className={`px-4 py-3 text-[11px] font-bold uppercase tracking-wide text-slate-500 ${c.centro ? 'text-center' : ''} ${c.precio ? 'text-right' : ''}`}>
                                    {c.label}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {items.map((item) => (
                            <tr key={item.id} className="transition-colors hover:bg-[rgb(var(--ab-acento-rgb)_/_0.04)]">
                                {cols.map((c) => (
                                    <td key={c.key} className={`px-4 py-3 align-middle text-slate-700 ${c.centro ? 'text-center' : ''} ${c.precio ? 'text-right' : ''}`}>
                                        <div className={`min-w-0 truncate ${c.principal ? 'font-semibold text-slate-900' : ''} ${c.mono ? 'font-mono text-[11px]' : ''}`} title={txt(item[c.key])}>
                                            <Celda item={item} col={c} />
                                        </div>
                                    </td>
                                ))}
