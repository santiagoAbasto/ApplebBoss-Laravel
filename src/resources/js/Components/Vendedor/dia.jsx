...
/* ─── Piezas de «Mi día» ──────────────────────────────────────────────────────
   El morado (#5C5E99) es el color del panel del vendedor y llega como variable
   CSS (--ab-acento) desde el armazón. Todo lo que se mueve se apaga solo con
...
export function TarjetaMeta({ total, meta, mes, faltaLabel = 'Te falta' }) {
  ...
  return (
    <motion.section
      ...
      className="ab-meta relative overflow-hidden rounded-2xl p-6 text-white"
      aria-label="Meta del mes"
    >
      {/* Fondo morado con profundidad y una aurora que respira */}
      <span aria-hidden="true" className="ab-meta-fondo absolute inset-0" />
      <span aria-hidden="true" className="ab-meta-aurora absolute -left-1/4 -top-1/2 h-[200%] w-[150%] opacity-80" />
      <span aria-hidden="true" className="absolute -right-16 -top-20 h-56 w-56 rounded-full bg-white/[0.07]" />
      <span aria-hidden="true" className="absolute -bottom-24 -left-10 h-48 w-48 rounded-full" style={{ background: 'rgba(1,20,70,0.3)' }} />
            ...
            <div className="mt-5 flex flex-wrap items-end justify-center gap-x-6 gap-y-3 text-center sm:justify-start sm:text-left">
              <div className="relative mx-auto aspect-[188/104] w-full max-w-[188px] shrink-0 sm:mx-0">
                <svg viewBox="0 0 188 104" className="absolute inset-0 h-full w-full" aria-hidden="true">
                  <defs>
                    <linearGradient id="ab-meta-arco" x1="0" y1="1" x2="1" y2="0">
                  ...
                  <motion.path
                    ...
                    fill="none" stroke="url(#ab-meta-arco)" strokeWidth="13" strokeLinecap="round"
                    ...
                    style={{ filter: 'drop-shadow(0 0 7px rgba(198,203,54,0.5))' }}
                  />
                </svg>
...
export function Numero({ icon: Icon, label, valor, hint, moneda = false, i = 0, tono = 'navy' }) {
  ...
  return (
    <Entrada i={i}>
      <div className="group h-full rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-all duration-300 hover:-translate-y-0.5 hover:border-[rgb(var(--ab-acento-rgb)_/_0.35)] hover:shadow-[0_16px_30px_-22px_rgb(var(--ab-acento-rgb)_/_0.9)]">
        ...
        <p className="mt-4 text-[24px] font-extrabold leading-none tracking-tight tabular-nums text-slate-900">
...
export function AccesoRapido({ href, icon: Icon, label, hint, i = 0 }) {
  ...
  return (
    ...
    >
      <Link
        href={href}
        className="group relative flex h-full items-center gap-3 overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-colors hover:border-[rgb(var(--ab-acento-rgb)_/_0.4)] hover:shadow-[0_18px_32px_-24px_rgb(var(--ab-acento-rgb)_/_1)]"
      >
        {/* Barrido de luz al pasar el mouse */}
        <span aria-hidden="true" className="ab-barrido pointer-events-none absolute inset-0" />
        <span className="relative grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)] transition-colors duration-300 group-hover:bg-[color:var(--ab-acento)] group-hover:text-white">
          <Icon className="h-5 w-5" />
        </span>
...
export function ListaReciente({ icon: Icon, titulo, verTodo, items, vacio, render, i = 0 }) {
  ...
  return (
    <Entrada i={i} className="h-full">
      <section className="flex h-full flex-col rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
        <header className="flex items-center gap-2.5 border-b border-slate-100 px-5 py-4">
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)]">
            <Icon className="h-4 w-4" />
          </span>
          <h2 className="flex-1 text-[15px] font-bold text-slate-900">{titulo}</h2>
          {verTodo && (
            <Link href={verTodo} className="group inline-flex items-center gap-1 text-xs font-semibold text-[color:var(--ab-acento)]">
              Ver todo
        ...
        {items.length === 0 ? (
          <p className="flex-1 px-5 py-8 text-center text-[13px] text-slate-400">{vacio}</p>
        ) : (
          <ul className="flex-1 divide-y divide-slate-100">
            {items.map((it, n) => (
              <motion.li
                ...
                className="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-[rgb(var(--ab-acento-rgb)_/_0.04)]"
              >
                {render(it)}
              </motion.li>
            ))}
