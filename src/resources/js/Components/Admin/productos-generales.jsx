...
function EstadoCodigo({ estado }) {
  if (estado === 'revisando') return <p className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-500"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Revisando el código…</p>;
    ...
    <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="border-b border-slate-100 px-5 py-4">
        <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
          <Package className="h-[18px] w-[18px] text-[#585E9F]" /> Resumen
        </h2>
      </div>

      <div className="space-y-4 p-5">
        <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#011446] text-white"><Package className="h-5 w-5" /></span>
          <div className="min-w-0 flex-1">
            <p className="truncate font-bold text-slate-900">{bonito(data.nombre) || 'Nuevo producto'}</p>
            <p className="truncate text-xs text-slate-500">{data.tipo ? tipoTexto(data.tipo) : 'Tipo de producto'}</p>
          </div>
