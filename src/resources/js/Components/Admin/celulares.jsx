...
export function CamposCelular({ form, sugerencias = {}, pasos = false, refs = {}, avisoEstado = null }) {
  ...
  return (
    <>
      ...
      <StepCard step={pasos ? 2 : undefined} icon={Fingerprint} title="Identificación" subtitle="Para garantías y control. Los IMEI no se muestran en la tienda; el número de serie sí.">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Número de serie (opcional)" error={errores.numero_serie}
            hint="Se muestra en la ficha de la tienda. No escribas aquí el IMEI.">
    ...
    <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="border-b border-slate-100 px-5 py-4">
        <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
          <Smartphone className="h-[18px] w-[18px] text-[#585E9F]" /> {titulo}
        </h2>
      </div>

      <div className="space-y-4 p-5">
        <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#011446] text-white"><Smartphone className="h-5 w-5" /></span>
          <div className="min-w-0 flex-1">
            <p className="truncate font-bold text-slate-900">{bonito(data.modelo) || 'Nuevo celular'}</p>
