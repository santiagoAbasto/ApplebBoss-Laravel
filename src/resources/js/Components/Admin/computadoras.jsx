...
/** Título que arma la tienda al publicar el equipo (nombre, chip, almacenamiento y color). */
...
export function BateriaMac({ valor }) {
  ...
  if (pct == null && ciclos == null) return <span className="text-slate-300">—</span>;
  return (
    <span className="inline-flex flex-col items-start gap-0.5">
      {pct != null && <Badge tone={pct >= 90 ? 'emerald' : pct >= 80 ? 'slate' : 'amber'} className="tabular-nums">{pct} %</Badge>}
      {ciclos != null && <span className="text-[11px] tabular-nums text-slate-500">{ciclos.toLocaleString('es-BO')} ciclos</span>}
    </span>
  );
}
...
export function CamposComputadora({ form, sugerencias = {}, pasos = false, refs = {}, avisoEstado = null }) {
  ...
  return (
    <>
          ...
          <Field label="Batería" error={errores.bateria}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="relative">
                ...
                <span className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-sm font-semibold text-slate-400">%</span>
              </div>
              <div className="relative">
                ...
                <span className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">ciclos</span>
              </div>
            </div>
          </Field>
        </div>
      ...
      <StepCard step={pasos ? 2 : undefined} icon={Fingerprint} title="Identificación" subtitle="El número de serie identifica al equipo y se muestra en la ficha de la tienda.">
      ...
      <StepCard step={pasos ? 3 : undefined} icon={Wallet} title="Precio y estado" subtitle="El precio de venta se usa al vender y al cotizar, y en la tienda si está publicado.">
...
/** Resumen en vivo: el equipo, cómo se titulará en la tienda y el precio con su ganancia. */
    ...
    <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="border-b border-slate-100 px-5 py-4">
        <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
          <Laptop className="h-[18px] w-[18px] text-[#585E9F]" /> Resumen
        </h2>
      </div>

      <div className="space-y-4 p-5">
        <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#011446] text-white"><Laptop className="h-5 w-5" /></span>
          <div className="min-w-0 flex-1">
            <p className="truncate font-bold text-slate-900">{bonito(data.nombre) || 'Nueva computadora'}</p>
            <p className="truncate text-xs text-slate-500">{detalleEquipo(data) || 'Chip, memoria y color'}</p>
          </div>
        ...
        {titulo && (
          <div className="rounded-xl border border-dashed border-slate-200 px-4 py-3">
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400">Título al publicarla en la tienda</p>
            <p className="mt-1 text-sm font-semibold text-slate-800">{titulo}</p>
          </div>
        )}
