...
export const checkCls = 'h-4 w-4 cursor-pointer rounded border-slate-300 text-[#011446] focus:ring-2 focus:ring-[rgb(var(--ab-acento-rgb)_/_0.3)] focus:ring-offset-0';

// Palabras con escritura propia (la misma idea que usa la tienda)
const PALABRAS = {
...
export function Nota({ children, tono = 'slate' }) {
  ...
  return (
    <p className={`mt-3 flex items-start gap-2 rounded-xl px-3 py-2.5 text-[13px] ${tonos[tono]}`}>
      <Icon className={`mt-0.5 h-4 w-4 shrink-0 ${tono === 'amber' ? 'text-amber-600' : 'text-[color:var(--ab-acento)]'}`} /> <span>{children}</span>
    </p>
  );
}
          ...
          return (
            <button key={o} type="button" aria-pressed={sel} onClick={() => onChange(o)}
              className={`h-10 rounded-xl border px-3.5 text-sm font-semibold tabular-nums transition-colors ${
                sel ? 'border-[#011446] bg-[#011446] text-white shadow-[0_8px_18px_-10px_rgba(1,20,70,0.6)]' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:text-slate-900'
              }`}>
              {o}
            </button>
          );
...
export function CampoCondicion({ valor, error, onChange }) {
  ...
  return (
    <Field label="Condición" error={error}
      hint={condicion ? `${condicion.ayuda} Es la que se muestra en la tienda.` : 'Es la que se muestra en la tienda.'}>
...
export function CampoProcedencia({ valor, error, onChange, sugerencias = [], lista = 'inventario-procedencias' }) {
  return (
    <>
      <Field label="Procedencia" error={error} hint="Proveedor o de dónde llegó. Es un dato interno: no se muestra en la tienda.">
...
export function Linea({ label, valor, mono = false }) {
  return (
    <div className="flex justify-between gap-3">
      ...
      <dd className={`min-w-0 truncate text-right font-semibold text-slate-900 ${mono ? 'font-mono text-[13px]' : ''}`}>{valor || '—'}</dd>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        <h1 className="truncate text-[32px] font-extrabold leading-tight tracking-tight text-[#011446]" style={{ fontFamily: "'Barlow Condensed', 'Barlow', sans-serif" }}>
          {titulo}
        </h1>
...
export function Stat({ icon: Icon, label, value, hint, tone = 'navy' }) {
  const tones = {
    ...
    lila: 'bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)]',
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      ...
      <p className="mt-4 text-[24px] font-extrabold leading-none tracking-tight tabular-nums text-slate-900">{value}</p>
...
export function PrecioConGanancia({ costo, venta }) {
  ...
  return (
    <div className="text-right">
      <p className="font-bold tabular-nums text-slate-900">{bsFmt(venta)}</p>
...
export function TiendaBadge({ publicado }) {
  if (publicado === undefined) return null;
  return publicado
    ? <Badge tone="emerald"><Store className="h-3 w-3" /> En la tienda</Badge>
    : <Badge tone="slate"><Store className="h-3 w-3" /> Borrador en tienda</Badge>;
}

// Los vendidos ya no importan para la tienda: solo se avisa la falta de condición en los demás
...
export function motivoEnListado(id, conHistorial, enTienda) {
  if (conHistorial.has(id)) return 'Figura en ventas o reservas: se conserva para no perder el historial.';
  if (enTienda && enTienda[id] !== undefined) return 'Está en Productos en la tienda: primero elimina su publicación.';
  return null;
}
...
export function AccionesFila({ editarUrl, nombre, estado, motivo, onBorrar, onHabilitar }) {
  return (
    <div className="flex justify-end gap-1.5">
      ...
      <Link href={editarUrl}
        className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-600 transition-colors hover:border-[#011446] hover:bg-[#011446] hover:text-white">
        <Pencil className="h-3.5 w-3.5" /> Editar
      </Link>
      {motivo ? (
        <span title={motivo} aria-label={`No se puede eliminar: ${motivo}`}
          className="grid h-8 w-8 cursor-not-allowed place-items-center rounded-lg border border-slate-100 bg-slate-50 text-slate-300">
          <Lock className="h-3.5 w-3.5" />
        </span>
      ) : (
        <button type="button" onClick={onBorrar} title="Eliminar" aria-label={`Eliminar ${nombre}`}
          className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-slate-400 transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-600">
...
export function Aviso({ tono = 'lila', icon: Icon = Store, children, accion = 'Ver cuáles', onAccion }) {
  const tonos = {
    lila: { caja: 'border-[rgb(var(--ab-acento-rgb)_/_0.25)] bg-[rgb(var(--ab-acento-rgb)_/_0.06)] text-slate-800', icono: 'text-[color:var(--ab-acento)]', boton: buttonCls('secondary', 'h-9 px-3') },
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-1.5" role="group" aria-label={etiqueta}>
      {filtros.map((f) => {
        ...
        const color = sel
          ? (f.alerta ? 'bg-amber-500 text-white' : 'bg-[#011446] text-white')
          : (f.alerta ? 'bg-amber-50 text-amber-800 hover:bg-amber-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-200');
        return (
...
export function BarraSeleccion({ cantidad, total, todosMarcados, hayMas, onSeleccionarTodos, onMarcar, onQuitar, aplicando }) {
  ...
  return (
    <div className="sticky bottom-4 z-30">
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl bg-[#011446] px-5 py-3 text-white shadow-[0_18px_40px_-18px_rgba(1,20,70,0.8)]">
...
/** `advertencia` explica qué pasa al borrar; en los módulos de la tienda no se borra nada del inventario. */
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-slate-500"><Icon className="h-5 w-5" /></span>
        <div className="min-w-0">
          <p className="truncate font-bold text-slate-900">{nombre}</p>
          {detalle && <p className="truncate text-xs text-slate-500">{detalle}</p>}
        </div>
      </div>
      <p className="mt-4 text-sm leading-relaxed text-slate-600">{advertencia}</p>
    </Modal>
  );
}
...
const MOVIMIENTOS = {
  venta: { icon: ShoppingCart, tone: 'bg-[#011446]/[0.07] text-[#011446]' },
...
function Movimiento({ m }) {
    ...
    <>
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-semibold text-slate-900">
          {m.titulo} {m.codigo && <span className="font-mono text-[color:var(--ab-acento)]">{m.codigo}</span>}
        </span>
  ...
  const cls = 'group -mx-2 flex items-center gap-3 rounded-xl px-2 py-2.5 transition-colors hover:bg-slate-50';
  return m.externo
  ...
  return (
    <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] sm:p-6">
      <div className="flex items-start gap-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)]"><History className="h-5 w-5" /></span>
        <div>
          <h2 className="text-base font-bold text-slate-900">Historial del equipo</h2>
          <p className="mt-0.5 text-[13px] text-slate-500">Ventas, permutas y reservas en las que figura.</p>
        </div>
      </div>
      {historial.length === 0 ? (
        <div className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-8 text-center text-sm text-slate-500">
...
const TONO_PUBLICACION = { Publicado: 'emerald', Borrador: 'slate', Programado: 'blue', Oculto: 'amber', Incompleto: 'amber' };

/** Estado en la tienda (publicar o editar la publicación) y eliminar el equipo. */
export function TiendaYEliminar({ publicacion, disponible, publicarUrl, bloqueo, onEliminar, sustantivo = 'equipo' }) {
  return (
    <section className="divide-y divide-slate-100 rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="flex flex-wrap items-center gap-4 p-5 sm:px-6">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)]"><Store className="h-5 w-5" /></span>
        <div className="min-w-0 flex-1">
          <p className="flex flex-wrap items-center gap-2 font-bold text-slate-900">
            Tienda {publicacion && <Badge tone={TONO_PUBLICACION[publicacion.estado] ?? 'slate'}>{publicacion.estado}</Badge>}
          </p>
          <p className="text-[13px] text-slate-500">
            {publicacion
              ? 'Tiene publicación en la tienda. El precio publicado sale de este inventario.'
              : disponible
                ? 'Todavía no está en la tienda. Al publicarlo eliges la condición y las fotos.'
                : 'Solo los equipos disponibles se pueden publicar.'}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {publicacion?.ver && (
            <a href={publicacion.ver} target="_blank" rel="noopener noreferrer" className={buttonCls('secondary', 'h-10')}>
              <ExternalLink className="h-4 w-4" /> Ver en la tienda
            </a>
          )}
          {publicacion ? (
            <Link href={publicacion.editar} className={buttonCls('secondary', 'h-10')}>Editar publicación</Link>
          ) : disponible && (
            <Link href={publicarUrl} className={buttonCls('primary', 'h-10')}>
              <Store className="h-4 w-4" /> Publicar en la tienda
            </Link>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 p-5 sm:px-6">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-red-50 text-red-600"><Trash2 className="h-5 w-5" /></span>
        <div className="min-w-0 flex-1">
          <p className="font-bold text-slate-900">Eliminar {sustantivo}</p>
          <p className="text-[13px] text-slate-500">{bloqueo ?? 'Se borra del inventario y no se puede deshacer.'}</p>
        </div>
        {bloqueo ? (
          <span className={buttonCls('secondary', 'h-10 cursor-not-allowed opacity-60')}><Lock className="h-4 w-4" /> No se puede eliminar</span>
        ) : (
