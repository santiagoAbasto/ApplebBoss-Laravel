...
export function ListaPermisos({ permisos = [], catalogo = [], max = 4, panelPropio = false }) {
  if (permisos.includes('*')) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#585E9F]">
  ...
  if (etiquetas.length === 0) {
    return panelPropio
      ? <span className="text-xs font-semibold text-slate-500">Su propio panel, en /vendedor</span>
...
export function SelectorPermisos({ catalogo = [], valor = [], onChange, bloqueado = false, panelPropio = false }) {
  ...
  if (bloqueado) {
    return (
      <p className="flex items-start gap-2 rounded-xl bg-[#585E9F]/[0.06] px-3.5 py-3 text-[13px] leading-relaxed text-slate-600">
        <Lock className="mt-0.5 h-4 w-4 shrink-0 text-[#585E9F]" />
  ...
  return (
    <div className="grid gap-3">
      {panelPropio && (
        <p className="flex items-start gap-2 rounded-xl bg-slate-50 px-3.5 py-3 text-[13px] leading-relaxed text-slate-600">
      ...
      {catalogo.map(({ grupo, modulos }) => (
        <div key={grupo} className="rounded-xl border border-slate-100">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-3.5 py-2">
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">{grupo}</p>
            <button type="button" onClick={() => alternarGrupo(modulos)}
              className="text-xs font-semibold text-[#585E9F] hover:underline">
          ...
          <div className="grid gap-1 p-2 sm:grid-cols-2">
            {modulos.map((m) => {
              ...
              return (
                <label key={m.clave}
                  className={`flex cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] transition-colors ${activo ? 'bg-[#585E9F]/[0.08] font-semibold text-slate-900' : 'text-slate-600 hover:bg-slate-50'}`}>
                  <span className={`grid h-4 w-4 shrink-0 place-items-center rounded border ${activo ? 'border-[#585E9F] bg-[#585E9F] text-white' : 'border-slate-300 bg-white'}`}>
...
export const CONSEJOS_USUARIOS = [
  {
    ...
    bien: 'Ayelen entra con su correo y su contraseña',
    mal: 'Todos entran con la cuenta del administrador',
  },
  {
    titulo: 'Da solo lo que necesita',
    texto: 'Quien atiende el mostrador no necesita ver los costos, los reportes ni la configuración de la tienda. Menos módulos es menos riesgo de tocar algo sin querer.',
