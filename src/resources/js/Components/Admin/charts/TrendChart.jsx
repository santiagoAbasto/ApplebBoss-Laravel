            ...
            {opciones.map((o) => {
                ...
                return (
                    <button key={o.key} type="button" role="tab" aria-selected={sel} title={o.title ?? o.label} onClick={() => onChange(o.key)}
                        className={`relative inline-flex h-7 items-center gap-1.5 rounded-lg px-2.5 text-xs font-bold transition-colors ${sel ? '' : 'hover:text-slate-900'}`}
        ...
        <ChartCard cargando={cargando}>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#585E9F]/10 text-[#585E9F]">
                        <ChartLine className="h-5 w-5" />
                    </span>
                    <div>
                        <p className="flex items-center gap-2 text-base font-bold text-slate-900">
                            Resumen económico
                            <span className="rounded-md bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold tracking-[0.12em] text-slate-500">BOB</span>
                        </p>
                        <p className="text-[13px] text-slate-500">{rango}{recortado && !esVelas ? ' · últimos 400 movimientos' : ''}</p>
                    </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    {hayDatos && (
                        <button type="button" onClick={() => setVista((v) => (v === 'grafico' ? 'tabla' : 'grafico'))}
                            className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-800"
                ...
                {METRICAS.map((m) => {
                    ...
                    return (
                        <button key={m.key} type="button" role="tab" aria-selected={sel} onClick={() => setMetrica(m.key)}
                            className={`relative rounded-lg py-2 text-sm font-bold transition-colors ${sel ? '' : 'hover:text-slate-900'}`}
                            style={{ color: sel ? '#FFFFFF' : '#475569' }}>
                            {sel && (
                                <motion.span layoutId={`${uid}-m`} className="absolute inset-0 rounded-lg shadow-[0_6px_16px_-8px_rgba(1,20,70,0.55)]"
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-2">
                <p className="text-[34px] font-extrabold leading-none tracking-tight text-slate-900 sm:text-[38px]">
            ...
            <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-[13px] text-slate-500">
                <span>Movimientos <strong className="font-bold text-slate-900">{cantidad.toLocaleString('es-BO')}</strong></span>
                {valores.length > 0 && <span>{meta.precio} promedio <strong className="font-bold text-slate-900">{bs(promedio)}</strong></span>}
                {valores.length > 0 && <span>Mayor <strong className="font-bold text-slate-900">{bs(mayor)}</strong></span>}
            </div>
            ...
            <div className="mt-5 flex flex-1 flex-col rounded-xl border p-3 sm:p-4" style={{ borderColor: T.borde, background: T.panel }}>
                {!hayDatos ? (
                    <div className="flex flex-1 flex-col items-center justify-center px-6 py-16 text-center">
                        <ChartLine className="h-6 w-6 text-slate-400" />
                        <p className="mt-3 text-sm font-bold text-slate-800">No hubo movimientos en este período</p>
                        <p className="mt-1 text-[13px] text-slate-500">Cuando registres ventas, servicios o egresos, aparecerán aquí.</p>
                    </div>
                ) : vista === 'tabla' ? (
                    <div className="max-h-[380px] overflow-auto rounded-lg border border-slate-200 bg-white">
                        <table className="min-w-full text-[13px]">
                            ...
                            <thead className="sticky top-0 bg-slate-50 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">
                                {esVelas ? (
                                    <tr>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {esVelas
                                    ? puntos.map((p, i) => velas[i] && (
                                        <tr key={p.fecha} className="hover:bg-slate-50/70">
                                            <th scope="row" className="px-3 py-2 text-left font-medium capitalize text-slate-700">{cortaBucket(p.fecha)}</th>
                                            {['o', 'h', 'l', 'c'].map((k) => <td key={k} className="px-3 py-2 text-right tabular-nums text-slate-700">{bs(velas[i][k])}</td>)}
                                            <td className="px-3 py-2 text-right font-bold tabular-nums text-slate-900">{bs(volumen[i])}</td>
                                        </tr>
                                    ))
                                    : [...ticks].reverse().map((m) => (
                                        <tr key={`${m.fecha}-${m.orden}`} className="hover:bg-slate-50/70">
                                            <td className="whitespace-nowrap px-3 py-2 text-slate-500">{dia(m.fecha).format('D MMM')}</td>
                                            <td className="max-w-[180px] truncate px-3 py-2 font-medium text-slate-900">{m.etiqueta}</td>
                                            {METRICAS.map((mm) => (
                    ...
                    <>
                        ...
                        <div className="mb-2 flex min-h-[28px] flex-wrap items-center gap-x-3 gap-y-1 px-1 text-[12px] text-slate-500" aria-hidden="true">
                            {lecturaVela ? (
                                <>
                                    <span className="font-bold capitalize text-slate-900">{largaBucket(puntos[lectura].fecha)}</span>
                                    {[['A', lecturaVela.o], ['Máx', lecturaVela.h], ['Mín', lecturaVela.l], ['C', lecturaVela.c]].map(([k, v]) => (
                                        <span key={k}>{k} <strong className="font-bold tabular-nums text-slate-900">{bs(v)}</strong></span>
                                    ))}
                                    <span>Total <strong className="font-bold tabular-nums text-slate-900">{bs(volumen[lectura])}</strong></span>
                                    <Chip sube={lecturaSube} />
                                </>
                            ) : lecturaTick ? (
                                <>
                                    <span className="font-bold text-slate-900">{dia(lecturaTick.fecha).format('D MMM YYYY')}</span>
                                    <span className="rounded-md bg-white px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-600 ring-1 ring-slate-200">
                                        {lecturaTick.tipo === 'egreso' ? 'Egreso' : lecturaTick.tipo === 'servicio' ? 'Servicio' : 'Venta'}
                                    </span>
                                    <span className="max-w-[220px] truncate text-slate-700">{lecturaTick.etiqueta}</span>
                                    <span>{meta.precio} <strong className="font-bold tabular-nums text-slate-900">{bs(valores[lectura])}</strong></span>
                                    <span>Acumulado <strong className="font-bold tabular-nums text-slate-900">{bs(acum[lectura])}</strong></span>
                                    {lectura > 0 && <Chip sube={lecturaSube} />}
                                </>
                            ) : esVelas && lectura >= 0 && puntos[lectura] ? (
                                <span><span className="font-bold capitalize text-slate-900">{largaBucket(puntos[lectura].fecha)}</span> · sin movimientos</span>
                            ) : null}
                        </div>
                        ...
                        <div
                            ...
                            className="relative rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-[#585E9F]/40"
                            style={{ height: H }}
                        >
                            {geo && (
            ...
            <p className="mt-3 text-xs text-slate-500">{esVelas ? `Cada vela resume los movimientos del ${unidad}: apertura, máximo, mínimo y cierre.` : meta.desc}</p>
        </ChartCard>
    );
}
