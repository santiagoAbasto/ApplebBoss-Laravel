...
export const CATEGORIAS = [
    ...
    { label: 'Productos Apple', color: '#eda100' },
        ...
        <ChartCard cargando={cargando}>
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-base font-bold text-slate-900">Ganancia por categoría</p>
                        <p className="text-[13px] text-slate-500">Cómo se reparte · antes de egresos</p>
                    </div>
                </div>
            ...
            {segmentos.segs.length === 0 ? (
                <div className="mt-6 flex flex-1 flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-6 py-16 text-center">
                    <PieChart className="h-6 w-6 text-slate-400" />
                    <p className="mt-3 text-sm font-bold text-slate-800">Todavía no hay ganancia en este período</p>
                    <p className="mt-1 text-[13px] text-slate-500">Aparecerá apenas registres ventas o servicios.</p>
                </div>
            ) : (
                <>
                    {/* Anillo */}
                            ...
                            <div className="w-[34%] text-center">
                                {sel ? (
                                    <motion.div key={sel.label} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18 }}>
                                        <p className="truncate text-[11px] font-semibold text-slate-500">{sel.label}</p>
                                        <p className="mt-0.5 text-[20px] font-extrabold leading-tight text-slate-900">{bs(sel.valor, 0)}</p>
                                        <p className="text-xs font-bold text-slate-600">{porcentaje.format(sel.frac)}</p>
                                    </motion.div>
                                ) : (
                                    <div>
                                        <p className="text-[11px] font-semibold text-slate-500">Ganancia neta</p>
                                        <p className="mt-0.5 text-[21px] font-extrabold leading-tight text-slate-900">
                                            {totalAnim < 0 ? `−${bs(Math.abs(totalAnim), 0)}` : bs(totalAnim, 0)}
                                        </p>
                                        <p className="text-[11px] text-slate-500">del período</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="mt-3 flex-1">
                        <div className="grid grid-cols-[minmax(0,1fr)_44px_92px] items-center gap-3 px-3 pb-2 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:grid-cols-[minmax(0,1fr)_84px_44px_92px]">
                            <span>Categoría</span>
                            ...
                            {lista.map((s, i) => {
                                ...
                                return (
                                    <motion.li key={s.label} tabIndex={0}
                                        ...
                                        className={`grid grid-cols-[minmax(0,1fr)_44px_92px] items-center gap-3 rounded-xl px-3 py-2 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[#585E9F]/30 sm:grid-cols-[minmax(0,1fr)_84px_44px_92px] ${activo ? 'bg-slate-50' : ''}`}>
                                        ...
                                        <span className="text-right text-xs font-bold tabular-nums text-slate-600">{porcentaje.format(s.frac)}</span>
                                        <span className="text-right text-sm font-bold tabular-nums text-slate-900">{bs(s.valor, 0)}</span>
                                    </motion.li>
                                );
                            })}
                            {perdidas.map((f) => (
                                <li key={f.label} className="grid grid-cols-[minmax(0,1fr)_auto_92px] items-center gap-3 rounded-xl px-3 py-2">
                                    ...
                                    <span className="text-right text-sm font-bold tabular-nums text-slate-900">−{bs(Math.abs(f.valor), 0)}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
