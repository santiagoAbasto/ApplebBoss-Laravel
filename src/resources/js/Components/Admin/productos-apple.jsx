...
// Piezas de las pantallas de productos Apple: iPad, AirPods, Apple Watch, Pencil y accesorios.
...
export const nombreEquipo = (p) => bonito(p?.modelo) || 'Producto Apple';
  ...
  return (
    <Field label={label} error={error}>
      <div className="relative">
        ...
        <span className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 text-[11px] font-semibold tabular-nums text-slate-400">{valor.length}/15</span>
      </div>
...
export function CamposProductoApple({ form, sugerencias = {}, pasos = false, refs = {}, avisoEstado = null }) {
  ...
  return (
    <>
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <div className="relative w-28">
                <Input value={data.sellado ? '100' : data.bateria_pct} disabled={data.sellado} inputMode="numeric" maxLength={3} placeholder="—" aria-label="Porcentaje de batería"
                  className="pr-9 tabular-nums disabled:bg-slate-50 disabled:text-slate-400"
      ...
      <StepCard step={pasos ? 2 : undefined} icon={Fingerprint} title="Identificación" subtitle="El número de serie se muestra en la ficha de la tienda; los IMEI no.">
        ...
        <div className="mt-4 flex items-center justify-between gap-4 rounded-xl border border-slate-200 px-4 py-3">
          <div>
            <p className="text-sm font-semibold text-slate-800">Tiene IMEI</p>
            <p className="text-xs text-slate-500">Solo los iPad o Apple Watch con conexión celular.</p>
          </div>
    ...
    <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <div className="border-b border-slate-100 px-5 py-4">
        <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
          <Apple className="h-[18px] w-[18px] text-[#585E9F]" /> Resumen
        </h2>
      </div>

      <div className="space-y-4 p-5">
        <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-[#011446] text-white"><Apple className="h-5 w-5" /></span>
          <div className="min-w-0 flex-1">
            <p className="truncate font-bold text-slate-900">{bonito(data.modelo) || 'Nuevo producto'}</p>
            <p className="truncate text-xs text-slate-500">{detalleEquipo(data) || 'Capacidad y color'}</p>
          </div>
