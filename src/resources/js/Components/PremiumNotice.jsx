        ...
        >
          <div className="flex items-start gap-3 rounded-2xl bg-white/95 p-3.5 shadow-[0_4px_8px_rgba(15,23,42,0.12)] ring-1 ring-black/10 backdrop-blur-md">
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              <p className="text-sm font-semibold leading-5 text-slate-900">{notice.title}</p>
              {notice.message && <p className="mt-0.5 text-sm leading-5 text-slate-600">{notice.message}</p>}
            </div>
            <button type="button" onClick={onClose} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-slate-500 hover:bg-slate-100 hover:text-slate-900" aria-label="Cerrar notificación">
              <X className="h-4 w-4" />
            </button>
          </div>
        </motion.div>
