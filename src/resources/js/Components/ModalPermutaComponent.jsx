...
export default function ModalPermutaComponent({ show, onClose, tipo, onGuardar }) {
  ...
  const campo = (c) => {
    ...
    return (
      <div key={c.name}>
        <label htmlFor={`permuta-${c.name}`} className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">
    ...
    <Transition.Root show={show} as={Fragment}>
      <Dialog as="div" className="relative z-[1060]" onClose={onClose}>
        <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0" enterTo="opacity-100" leave="ease-in duration-150" leaveFrom="opacity-100" leaveTo="opacity-0">
          <div className="fixed inset-0 bg-[#011446]/40 backdrop-blur-[2px]" />
        </Transition.Child>
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <Transition.Child as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 translate-y-2 scale-[0.98]" enterTo="opacity-100 translate-y-0 scale-100" leave="ease-in duration-150" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-[0.98]">
              <Dialog.Panel className="w-full max-w-2xl overflow-hidden rounded-2xl bg-white text-left shadow-2xl" style={{ fontFamily: "'Barlow', system-ui, sans-serif" }}>
                <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-4">
                  <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#585E9F]/10 text-[#585E9F]"><Repeat className="h-5 w-5" /></span>
                    <div>
                      <Dialog.Title className="text-base font-bold text-slate-900">Equipo que entrega el cliente</Dialog.Title>
                      <p className="text-[13px] text-slate-500">Tipo: {TITULOS[tipo] ?? '—'} · entra al inventario como permuta</p>
                    </div>
                  </div>
                  <button type="button" onClick={onClose} aria-label="Cerrar" className="grid h-9 w-9 place-items-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                ...
                <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
                  ...
                  <div className="mt-5 grid gap-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4 sm:grid-cols-2">
                    <div>
                      <label htmlFor="permuta-precio_costo" className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">
                      ...
                      <p className="mt-1 text-xs text-slate-500">Se descuenta del total de la venta.</p>
                    </div>
                    <div>
                      <label htmlFor="permuta-precio_venta" className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">
                      ...
                      <p className="mt-1 text-xs text-slate-500">A cuánto lo venderás después.</p>
                    </div>
                  </div>
                ...
                <div className="flex justify-end gap-3 border-t border-slate-100 bg-slate-50/60 px-6 py-4">
                  ...
                  <button type="button" onClick={handleGuardar}
                    className="inline-flex items-center rounded-xl bg-[#011446] px-5 py-2 text-sm font-semibold text-white shadow-[0_8px_18px_-10px_rgba(1,20,70,0.6)] hover:bg-[#0A1F5C]">
                    Guardar equipo
                  </button>
                </div>
              </Dialog.Panel>
