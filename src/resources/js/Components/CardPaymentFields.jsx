  ...
  return (
    <div>
      ...
      {error
        ...
        : n > 0 && n < 4 ? <p className="mt-1 text-xs text-slate-400">Faltan {4 - n} {4 - n === 1 ? 'número' : 'números'}</p> : null}
    </div>
  );
}
    ...
    <div className="md:col-span-2 grid grid-cols-1 items-center gap-5 lg:grid-cols-[minmax(0,360px)_1fr]">
      ...
      >
        <motion.div
          ...
          className="relative aspect-[1.586/1] w-full select-none overflow-hidden rounded-[18px] text-white shadow-[0_26px_50px_-24px_rgba(1,20,70,0.85)] ring-1 ring-white/10"
          style={{
            background: 'linear-gradient(135deg, #011446 0%, #0A1F5C 46%, #2B2A6B 78%, #3A3F86 100%)',
            ...(reduce ? {} : { rotateX: rotX, rotateY: rotY, transformStyle: 'preserve-3d' }),
          }}
        >
          {/* Fondo: círculos de marca, textura y brillo */}
          <span aria-hidden="true" className="absolute -right-16 -top-20 h-56 w-56 rounded-full" style={{ background: 'radial-gradient(circle, rgba(123,130,216,0.55) 0%, rgba(88,94,159,0.25) 55%, transparent 70%)' }} />
          <span aria-hidden="true" className="absolute -bottom-14 -left-8 h-36 w-36 rounded-full" style={{ background: 'radial-gradient(circle, rgba(198,203,54,0.28) 0%, transparent 68%)' }} />
          <Guilloche />
          ...
          <div className="relative flex h-full flex-col justify-between p-5">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[19px] font-extrabold leading-none tracking-tight" style={{ fontFamily: DISPLAY }}>Apple Boss</span>
              </div>
              <SinContacto />
            </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        <p className="text-sm font-bold text-slate-900">Datos de la tarjeta</p>
        <p className="mt-0.5 text-[13px] text-slate-500">Anota los 4 primeros y los 4 últimos números; aparecen en la nota.</p>
        ...
        <p className="mt-4 flex items-start gap-2 rounded-xl bg-slate-50 px-3 py-2.5 text-xs leading-relaxed text-slate-500">
          <ShieldCheck className="mt-px h-4 w-4 shrink-0 text-[#585E9F]" />
