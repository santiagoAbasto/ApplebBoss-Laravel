    ...
    return (
        <div>
            ...
            <div className="relative">
                {Icon && <Icon className="pointer-events-none absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-400" aria-hidden="true" />}
                <input
                    id={id}
                    name={id}
                    type={inputType}
                    ...
                    className={`h-12 w-full rounded-xl border bg-white text-[15px] text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:ring-4 ${
                        error
                            ? 'border-red-400 focus:border-red-500 focus:ring-red-100'
                            : 'border-slate-200 focus:border-[#585E9F] focus:ring-[#585E9F]/15'
                    }`}
            ...
            {error ? (
                <p id={`${id}-error`} role="alert" className="mt-1.5 flex items-start gap-1.5 text-[13px] font-medium text-red-600">
                    <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" /> {error}
                </p>
            ) : hint ? (
                <p id={`${id}-hint`} className="mt-1.5 text-[13px] text-slate-500">{hint}</p>
            ) : null}
            {children}
        </div>
    );
...
/** Botón principal del acceso (azul de marca) con estado de carga. */
export function AuthButton({ children, loading, loadingText, className = '', ...props }) {
    return (
        <button
            type="submit"
            disabled={loading || props.disabled}
            className={`group inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl text-[15px] font-bold text-white shadow-[0_10px_24px_-12px_rgba(1,20,70,0.6)] transition hover:brightness-125 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#C6CB36]/60 disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
...
export function AuthAlert({ tone = 'success', children }) {
    const tones = {
        ...
        info:    'border-[#585E9F]/25 bg-[#585E9F]/[0.07] text-[#28224F]',
    };
    const Icon = tone === 'error' ? AlertCircle : CheckCircle2;
    return (
        <div role="status" className={`mb-6 flex items-start gap-2.5 rounded-xl border px-4 py-3 text-sm font-medium ${tones[tone]}`}>
...
export function AuthCheckbox({ id, checked, onChange, children }) {
    return (
        <label htmlFor={id} className="flex cursor-pointer select-none items-center gap-2.5 text-sm text-slate-600">
            <input id={id} type="checkbox" checked={checked} onChange={onChange}
                className="h-[18px] w-[18px] rounded-md border-slate-300 text-[#011446] focus:ring-[#585E9F]/30" />
            {children}
        </label>
    );
}
...
export function PasswordStrength({ password }) {
    ...
    const nivel = ok <= 2 ? { label: 'Débil', color: '#DC2626' } : ok <= 4 ? { label: 'Casi lista', color: '#D97706' } : { label: 'Segura', color: '#16A34A' };
        ...
        <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3">
            <div className="flex items-center gap-3">
                <div className="flex flex-1 gap-1" aria-hidden="true">
                    {REGLAS.map((_, i) => (
                        <span key={i} className="h-1.5 flex-1 rounded-full transition-colors duration-300" style={{ background: i < ok ? nivel.color : '#E2E8F0' }} />
                    ))}
                </div>
            ...
            <ul className="mt-2.5 grid grid-cols-2 gap-x-3 gap-y-1">
                {REGLAS.map((r) => {
                    ...
                    return (
                        <li key={r.label} className={`flex items-center gap-1.5 text-xs ${cumple ? 'text-emerald-700' : 'text-slate-500'}`}>
...
export function PasswordMatch({ password, confirmation }) {
    ...
    return (
        <p className={`mt-1.5 flex items-center gap-1.5 text-[13px] font-medium ${igual ? 'text-emerald-700' : 'text-red-600'}`}>
...
export const linkCls = 'font-semibold text-[#585E9F] underline-offset-4 transition-colors hover:text-[#011446] hover:underline';
