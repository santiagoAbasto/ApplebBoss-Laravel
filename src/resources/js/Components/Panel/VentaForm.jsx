...
export default function VentaForm({ celulares, computadoras, productosGenerales, reservasActivas = [], Layout, prefijo = 'admin' }) {
  ...
  return (
    <Layout title="Nueva venta">
      <Head title="Nueva venta | Apple Boss" />
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Completa los pasos. Al registrar se abre la nota para imprimir.</p>
          </div>
        </div>
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="min-w-0 space-y-5">
            ...
            {reservasActivas.length > 0 && (
              <StepCard icon={CalendarCheck} title="¿Viene de una reserva?" subtitle="Si el cliente ya dejó un abono, elige su reserva y se cargan sus productos.">
                <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
                  ...
                  {reservaSeleccionada && (
                    <div className="rounded-xl border border-[rgb(var(--ab-acento-rgb)_/_0.25)] bg-[rgb(var(--ab-acento-rgb)_/_0.06)] px-4 py-2.5 text-sm text-[#28224F]">
            ...
            <StepCard step={2} title="Productos" subtitle="Elige el tipo y busca por código, IMEI, serie o nombre.">
              ...
              {/* Productos agregados */}
              <div className="mt-5">
                <p className="mb-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400">En esta venta · {items.length}</p>
                {items.length === 0 ? (
            ...
            <StepCard step={3} title="Permuta (opcional)" subtitle="Actívala si el cliente entrega un equipo como parte de pago."
              ...
              {!esPermuta ? (
                <p className="text-[13px] text-slate-500">Sin permuta: el cliente paga el total.</p>
              ) : (
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
                <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                  <ShoppingCart className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Resumen de la venta
                </h2>
              ...
              <div className="space-y-4 p-5">
                ...
                <dl className="space-y-1.5 border-t border-slate-100 pt-4 text-sm">
                  <div className="flex justify-between gap-3"><dt className="text-slate-500">Subtotal</dt><dd className="font-semibold tabular-nums text-slate-900">{bsFmt(subtotalItems)}</dd></div>
                  {valorPermuta > 0 && (
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Permuta</dt><dd className="font-semibold tabular-nums text-rose-600">−{bsFmt(valorPermuta)}</dd></div>
                  )}
                  {montoReserva > 0 && (
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Abono de la reserva</dt><dd className="font-semibold tabular-nums text-rose-600">−{bsFmt(montoReserva)}</dd></div>
                  )}
                </dl>
                ...
                <p className="text-center text-xs text-slate-400">Se descuenta del inventario y se abre la nota.</p>
              </div>
            </section>
          </aside>
        </div>
