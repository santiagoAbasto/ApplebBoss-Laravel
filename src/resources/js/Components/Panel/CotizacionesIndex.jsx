  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5" role="group" aria-label={label}>
      <span className="mr-1.5 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.1em] text-slate-400">
        <Icon className="h-4 w-4 text-[color:var(--ab-acento)]" /> {label}
      </span>
      {options.map((o) => (
        <button key={o.key} type="button" onClick={() => onChange(o.key)} aria-pressed={value === o.key}
          className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${value === o.key ? 'bg-[#011446] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
          {o.label}
          {o.n !== undefined && <span className={value === o.key ? 'text-white/70' : 'text-slate-400'}>{o.n}</span>}
        </button>
      ))}
    </div>
  );
    ...
    <div className="flex flex-wrap justify-end gap-1.5">
      ...
      {whatsappValido(c.telefono) ? (
        ...
        <span title="La cotización no tiene número de WhatsApp"
          className="inline-flex h-8 cursor-not-allowed items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 text-xs font-bold text-slate-400">
          <MessageCircle className="h-3.5 w-3.5" /> WhatsApp
        </span>
      )}
      {c.correo_cliente && (
...
export default function CotizacionesIndex({ cotizaciones = [], Layout, prefijo = 'admin', titulo, subtitulo, guia = null }) {
  ...
  return (
    <Layout title="Cotizaciones">
      <Head title="Cotizaciones | Apple Boss" />
      <Toast toast={toast} />
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
              <Receipt className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Detalle de cotizaciones
            </h2>
            ...
            <>
                ...
                <table className="w-full min-w-[1120px] text-[13px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {visibles.map((c) => {
                      ...
                      return (
                        <tr key={c.id} className={`align-top transition-colors ${marcada ? 'bg-[rgb(var(--ab-acento-rgb)_/_0.06)]' : 'hover:bg-slate-50/70'}`}>
                          ...
                          <td className="px-3 py-3">
                            <p className="font-mono text-[13px] font-bold text-[color:var(--ab-acento)]">{numeroCotizacion(c.id)}</p>
                          ...
                          <td className="px-4 py-3">
                            <p className="max-w-[220px] truncate font-semibold text-slate-900">{c.nombre_cliente}</p>
                          ...
                          <td className="px-4 py-3">
                            <p className="max-w-[260px] truncate font-medium text-slate-800" title={c.items.map(nombreItem).join(' · ')}>
              ...
              <ul className="divide-y divide-slate-100 md:hidden">
                {visibles.map((c) => {
                  const marcada = seleccion.includes(c.id);
                  return (
                    <li key={c.id} className={`space-y-3 px-4 py-4 ${marcada ? 'bg-[rgb(var(--ab-acento-rgb)_/_0.06)]' : ''}`}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-semibold text-slate-900">{c.nombre_cliente}</p>
                          <p className="font-mono text-xs font-bold text-[color:var(--ab-acento)]">
                            {numeroCotizacion(c.id)} <span className="font-sans font-normal text-slate-400">· {fechaCorta(c.fecha_cotizacion || c.created_at)}</span>
                          </p>
                        </div>
                        <EnvioEstado c={c} />
                      </div>
              ...
              {filtradas.length > limite && (
                <div className="border-t border-slate-100 px-5 py-3 text-center">
                  <button type="button" onClick={() => setLimite((l) => l + 50)} className={buttonCls('secondary')}>
                    Mostrar 50 más <span className="text-slate-400">({(filtradas.length - limite).toLocaleString('es-BO')} restantes)</span>
                  </button>
                </div>
              )}
            </>
      ...
      {reenviar && (
        ...
        >
          <p className="text-sm text-slate-600">Se vuelve a enviar el PDF de la cotización a este correo:</p>
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
            <p className="font-semibold text-slate-900">
              {reenviar.nombre_cliente} <span className="font-mono text-[color:var(--ab-acento)]">· {numeroCotizacion(reenviar.id)}</span>
            </p>
            <p className="mt-0.5 text-slate-500">{reenviar.correo_cliente}</p>
          </div>
        </Modal>
      )}
    </Layout>
