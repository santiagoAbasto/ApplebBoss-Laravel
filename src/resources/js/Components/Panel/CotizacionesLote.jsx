...
export default function CotizacionesLote({ links = [], omitidas = [], Layout, prefijo = 'admin' }) {
  ...
  return (
    <Layout title="Enviar cotizaciones por WhatsApp">
      <Head title="Enviar cotizaciones por WhatsApp | Apple Boss" />
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Cada botón abre el chat del cliente con el mensaje listo; solo falta presionar enviar.</p>
          </div>
        </div>

        {links.length > 0 && (
          <div className="flex items-start gap-3 rounded-2xl border border-slate-200/80 bg-white px-5 py-4 text-sm shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
            <Info className="mt-0.5 h-4 w-4 shrink-0 text-[color:var(--ab-acento)]" />
            <div className="space-y-1">
              <p className="font-semibold text-slate-900">
                {links.length} {links.length === 1 ? 'mensaje listo' : 'mensajes listos'} · {abiertos.length} {abiertos.length === 1 ? 'abierto' : 'abiertos'}
              </p>
              <p className="text-slate-500">WhatsApp abre un chat a la vez: ve uno por uno. Los que ya abriste quedan marcados.</p>
              {omitidas.length > 0 && (
        ...
        {links.length === 0 ? (
          ...
          <ul className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {links.map((item) => {
              ...
              return (
                <li key={item.id}
                  className={`flex flex-col rounded-2xl border bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-colors ${abierto ? 'border-emerald-300' : 'border-slate-200/80'}`}>
                  ...
                  <div className="mt-4 flex-1 whitespace-pre-wrap break-words rounded-xl bg-slate-50 p-3.5 text-[13px] leading-relaxed text-slate-700">
                    {item.mensaje}
                  </div>
