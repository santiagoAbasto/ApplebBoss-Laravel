...
const MOVIMIENTOS = {
  ...
  cotizacion: { icon: FileText, tone: 'bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)]' },
...
function Cifra({ icon: Icon, label, valor, extra }) {
  return (
    <div className="rounded-xl border border-slate-100 bg-slate-50/60 px-4 py-3">
      <p className="flex items-center gap-1.5 text-xs font-semibold text-slate-500"><Icon className="h-3.5 w-3.5" /> {label}</p>
      <p className="mt-1.5 text-xl font-extrabold leading-none tabular-nums text-slate-900">{valor}</p>
      {extra && <p className="mt-1 truncate text-xs tabular-nums text-slate-500">{extra}</p>}
    </div>
  );
}
...
export default function ClienteForm({ cliente, actividad = {}, Layout, prefijo = 'admin' }) {
  ...
  return (
    <Layout title={`Editar cliente · ${cliente.nombre}`}>
      <Head title={`Editar cliente · ${cliente.nombre} | Apple Boss`} />
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-500">Sus datos de contacto se usan en ventas, cotizaciones y promociones.</p>
          </div>
        </div>
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="min-w-0 space-y-5">
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] sm:p-6">
              <div className="flex items-start gap-3">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[rgb(var(--ab-acento-rgb)_/_0.1)] text-[color:var(--ab-acento)]"><Receipt className="h-5 w-5" /></span>
                <div>
                  <h2 className="text-base font-bold text-slate-900">Actividad del cliente</h2>
                  <p className="mt-0.5 text-[13px] text-slate-500">Compras, servicios técnicos, cotizaciones y reservas registradas con sus datos.</p>
                </div>
              </div>
              ...
              {ventas.ultima && <p className="mt-3 text-xs text-slate-500">Última compra: {fmtFecha(ventas.ultima, true)}</p>}

              <h3 className="mt-6 text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Movimientos recientes</h3>
              {recientes.length === 0 ? (
                <div className="mt-2 rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-8 text-center text-sm text-slate-500">
              ...
              <p className="mt-3 text-[11px] text-slate-400">Las ventas y reservas se encuentran por el número de teléfono.</p>
            </section>
          </div>
          ...
          <aside className="xl:sticky xl:top-24">
            <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="flex items-center gap-3">
                <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#011446] text-base font-bold text-white">
                  {iniciales(data.nombre || cliente.nombre)}
                </span>
                <div className="min-w-0">
                  <p className="truncate text-base font-bold text-slate-900">{data.nombre.trim() || cliente.nombre}</p>
                  <p className="text-xs text-slate-500">Cliente desde {fmtFecha(cliente.created_at, true)}</p>
                </div>
              </div>
