...
export default function ReservasIndex({ reservas = [], Layout, prefijo = 'admin', titulo, subtitulo, guia = null }) {
    ...
    <div className="flex flex-wrap justify-end gap-1.5">
      {r.estado === 'activa' && (
        <Link href={route(`${prefijo}.ventas.create`, { reserva_id: r.id })}
          className="inline-flex h-8 items-center gap-1.5 rounded-lg bg-[#011446] px-3 text-xs font-bold text-white transition-colors hover:bg-[#0A1F5C]">
          <ShoppingCart className="h-3.5 w-3.5" /> Vender
        </Link>
      )}
      {r.estado === 'vendida' && r.venta && (
        <Link href={route(`${prefijo}.ventas.edit`, r.venta.id)}
          className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 text-xs font-semibold text-slate-600 hover:border-slate-300 hover:text-slate-900">
          Ver venta <span className="font-mono text-[color:var(--ab-acento)]">{r.venta.codigo_nota}</span>
        </Link>
      )}
      ...
      {r.estado === 'activa' && (
        <>
          <button type="button" onClick={() => setConfirmar({ reserva: r, estado: 'vencida' })} title="Marcar como vencida" aria-label={`Marcar ${r.codigo_nota} como vencida`}
            className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-amber-600 transition-colors hover:border-amber-300 hover:bg-amber-50">
          ...
          <button type="button" onClick={() => setConfirmar({ reserva: r, estado: 'cancelada' })} title="Cancelar reserva" aria-label={`Cancelar ${r.codigo_nota}`}
            className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-rose-600 transition-colors hover:border-rose-300 hover:bg-rose-50">
  ...
  return (
    <Layout title="Reservas">
      <Head title="Reservas | Apple Boss" />
      <Toast toast={toast} />
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {[['todas', 'Todas', reservas.length], ...Object.entries(ESTADOS).map(([k, e]) => [k, e.label, conteo[k] || 0])].map(([k, label, n]) => (
              <button key={k} type="button" onClick={() => setEstado(k)}
                className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${estado === k ? 'bg-[#011446] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {label} <span className={estado === k ? 'text-white/70' : 'text-slate-400'}>{n}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
              <CalendarCheck className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Detalle de reservas
            </h2>
            {hayFiltros && (
                ...
                <table className="w-full min-w-[1040px] text-[13px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filtradas.map((r) => {
                      const items = r.items || [];
                      return (
                        <tr key={r.id} className="align-top transition-colors hover:bg-slate-50/70">
                          <td className="px-5 py-3">
                            <p className="font-mono text-[13px] font-bold text-[color:var(--ab-acento)]">{r.codigo_nota}</p>
                            <p className="mt-0.5 whitespace-nowrap text-xs text-slate-400">{fecha(r.fecha || r.created_at)}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="max-w-[180px] truncate font-semibold text-slate-900">{r.nombre_cliente}</p>
                          ...
                          <td className="px-4 py-3">
                            <p className="max-w-[220px] truncate font-medium text-slate-800">{items[0] ? nombreItem(items[0]) : '—'}</p>
                            {items.length > 1 && <p className="mt-0.5 text-xs text-slate-400">y {items.length - 1} más</p>}
                          </td>
                          <td className="px-4 py-3 text-right tabular-nums text-slate-700">{bsFmt(r.subtotal)}</td>
                          <td className="px-4 py-3 text-right tabular-nums font-semibold text-emerald-700">{bsFmt(r.monto_reserva)}</td>
                          <td className="px-4 py-3 text-right tabular-nums font-bold text-slate-900">{bsFmt(saldoDe(r))}</td>
                  ...
                  <li key={r.id} className="space-y-3 px-4 py-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate font-semibold text-slate-900">{r.nombre_cliente}</p>
                        <p className="font-mono text-xs font-bold text-[color:var(--ab-acento)]">{r.codigo_nota} <span className="font-sans font-normal text-slate-400">· {fecha(r.fecha || r.created_at)}</span></p>
                      </div>
                      <EstadoBadge estado={r.estado} />
                    </div>
                    <div className="grid grid-cols-3 gap-2 rounded-xl bg-slate-50 p-3 text-center">
                      <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Total</p><p className="text-sm font-semibold tabular-nums text-slate-800">{bsFmt(r.subtotal)}</p></div>
                      <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Abono</p><p className="text-sm font-semibold tabular-nums text-emerald-700">{bsFmt(r.monto_reserva)}</p></div>
                      <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Saldo</p><p className="text-sm font-bold tabular-nums text-slate-900">{bsFmt(saldoDe(r))}</p></div>
                    </div>
                    <Acciones r={r} />
                  </li>
                ))}
      ...
      {conf && (
        ...
        >
          <p className="text-sm text-slate-600">{conf.texto}</p>
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
            <p className="font-semibold text-slate-900">{confirmar.reserva.nombre_cliente} <span className="font-mono text-[color:var(--ab-acento)]">· {confirmar.reserva.codigo_nota}</span></p>
            <p className="mt-0.5 text-slate-500">Abono registrado: {bsFmt(confirmar.reserva.monto_reserva)}</p>
          </div>
        </Modal>
      )}
    </Layout>
