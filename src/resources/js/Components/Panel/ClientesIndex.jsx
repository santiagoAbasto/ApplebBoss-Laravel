...
const MENSAJE_BASE = 'Hola {nombre}, en Apple Boss tenemos promociones especiales para ti. No te las pierdas: visítanos o escríbenos ahora mismo.';
...
const checkCls = 'h-4 w-4 cursor-pointer rounded border-slate-300 text-[#011446] focus:ring-2 focus:ring-[rgb(var(--ab-acento-rgb)_/_0.3)] focus:ring-offset-0';
    ...
    <div className="flex justify-end gap-1.5">
      {c.wa ? (
        <a href={`https://wa.me/${c.wa}`} target="_blank" rel="noopener noreferrer" title="Abrir chat de WhatsApp" aria-label={`Abrir WhatsApp de ${c.nombre}`}
          className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-emerald-600 transition-colors hover:border-emerald-300 hover:bg-emerald-50">
          <MessageCircle className="h-3.5 w-3.5" />
        </a>
      ) : (
        <span title="Sin número de WhatsApp válido" className="grid h-8 w-8 cursor-not-allowed place-items-center rounded-lg border border-slate-100 bg-slate-50 text-slate-300">
          <MessageCircle className="h-3.5 w-3.5" />
        </span>
      )}
      ...
      {paso === 'mensaje' ? (
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            Se prepara para <span className="font-bold text-slate-900">{destinatarios.length}</span>{' '}
          ...
          {ejemplo && mensaje.trim() && (
            <div className="rounded-2xl bg-emerald-50/70 p-4">
              ...
              <div className="mt-2 max-w-md whitespace-pre-wrap rounded-2xl rounded-tl-sm bg-white px-4 py-3 text-sm leading-relaxed text-slate-800 shadow-sm">
        ...
        <div>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-slate-600">WhatsApp abre un chat a la vez: presiona «Abrir» en cada cliente y luego enviar en WhatsApp.</p>
          ...
          <ul className="mt-3 max-h-[50vh] divide-y divide-slate-100 overflow-y-auto rounded-xl border border-slate-200">
            {destinatarios.map((c) => {
...
export default function ClientesIndex({ clientes = [], Layout, prefijo = 'admin', titulo, subtitulo, guia = null }) {
  ...
  return (
    <Layout title="Clientes">
      <Head title="Clientes | Apple Boss" />
      <Toast toast={toast} />
          ...
          <div className="mt-3 flex flex-wrap gap-1.5" role="group" aria-label="Filtrar clientes">
            {FILTROS.map((fx) => (
              <button key={fx.key} type="button" onClick={() => setFiltro(fx.key)} aria-pressed={filtro === fx.key}
                className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${filtro === fx.key ? 'bg-[#011446] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                {fx.label} <span className={filtro === fx.key ? 'text-white/70' : 'text-slate-400'}>{conteo[fx.key].toLocaleString('es-BO')}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <section ref={tablaRef} className="scroll-mt-24 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
              <Users className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Listado de clientes
            </h2>
            <div className="flex items-center gap-2">
              {visibles.length > 0 && (
                <label className="flex cursor-pointer items-center gap-2 text-xs font-semibold text-slate-600 md:hidden">
            ...
            <>
                ...
                <table className="w-full min-w-[900px] text-[13px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {visibles.map((c) => {
                      ...
                      return (
                        <tr key={c.id} className={`transition-colors ${marcado ? 'bg-[rgb(var(--ab-acento-rgb)_/_0.06)]' : 'hover:bg-slate-50/70'}`}>
                          <td className="py-3 pl-5 pr-2">
                            <input type="checkbox" checked={marcado} onChange={() => alternar(c.id)} aria-label={`Seleccionar a ${c.nombre}`} className={checkCls} />
                          </td>
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-3">
                              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#011446]/[0.07] text-xs font-bold text-[#011446]">{iniciales(c.nombre)}</span>
                              <div className="min-w-0">
                                <p className="max-w-[260px] truncate font-semibold text-slate-900">{c.nombre}</p>
                                <p className="text-xs text-slate-400">Desde {fechaCorta(c.created_at)}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            {c.telefono ? (
                              <span className="inline-flex items-center gap-2">
                                <span className="tabular-nums text-slate-700">{c.telefono}</span>
                                {!c.wa && <Badge tone="amber">Revisar</Badge>}
                              </span>
                            ) : <span className="text-slate-300">—</span>}
                          </td>
                          <td className="px-4 py-3">
                            {c.correo ? <span className="max-w-[240px] truncate text-slate-700">{c.correo}</span> : <span className="text-slate-300">—</span>}
                          </td>
                          {creadores.length > 1 && <td className="px-4 py-3 text-slate-600">{c.usuario?.name || '—'}</td>}
              ...
              <ul className="divide-y divide-slate-100 md:hidden">
                {visibles.map((c) => {
                  const marcado = seleccion.includes(c.id);
                  return (
                    <li key={c.id} className={`flex items-center gap-3 px-4 py-3.5 ${marcado ? 'bg-[rgb(var(--ab-acento-rgb)_/_0.06)]' : ''}`}>
                      <input type="checkbox" checked={marcado} onChange={() => alternar(c.id)} aria-label={`Seleccionar a ${c.nombre}`} className={checkCls} />
                      <div className="min-w-0 flex-1">
                        <p className="truncate font-semibold text-slate-900">{c.nombre}</p>
                        <p className="truncate text-xs text-slate-500">{[c.telefono, c.correo].filter(Boolean).join(' · ') || 'Sin contacto'}</p>
                      </div>
