  ...
  return (
    ...
    >
      <div className="space-y-4">
        <p className="rounded-xl bg-slate-50 px-4 py-3 text-[13px] leading-relaxed text-slate-600">
          <span className="font-semibold text-slate-900">{servicio.equipo}</span> de {servicio.cliente}
        ...
        {guardados ? (
          <ul className="space-y-2">
            {guardados.map((t, i) => (
              <li key={i} className="grid grid-cols-[minmax(0,1fr)_140px] items-center gap-3 rounded-xl border border-slate-200 px-3.5 py-2.5">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-slate-900">{t.descripcion || 'Trabajo sin descripción'}</p>
                  <p className="text-xs text-slate-500">El cliente paga {bsFmt(Number(t.precio) || 0)}</p>
                </div>
        ...
        <dl className="grid grid-cols-3 gap-2 rounded-xl bg-slate-50 p-3 text-center">
          <div><dt className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Cobrado</dt><dd className="text-sm font-bold tabular-nums text-slate-900">{bsFmt(cobrado)}</dd></div>
          <div><dt className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Costo</dt><dd className="text-sm font-semibold tabular-nums text-slate-600">{completo ? bsFmt(costo) : '—'}</dd></div>
          <div>
            <dt className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Utilidad</dt>
            <dd className={`text-sm font-bold tabular-nums ${!completo ? 'text-slate-400' : utilidad < 0 ? 'text-rose-600' : 'text-emerald-700'}`}>
...
export default function ServiciosIndex({ servicios = [], filtros = {}, vendedores = [], tecnicos = [], pendientesDeCosto = 0, Layout, prefijo = 'admin', titulo, subtitulo, guia = null }) {
  ...
  return (
    <Layout title="Servicios técnicos">
      <Head title="Servicios técnicos | Apple Boss" />
      <Toast toast={toast} />
          ...
          <div className="mt-3 flex flex-col gap-3 border-t border-slate-100 pt-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="mr-1.5 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.1em] text-slate-400">
                <CalendarRange className="h-4 w-4 text-[color:var(--ab-acento)]" /> Período
              </span>
              {periodos.map((p) => (
            ...
            <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-slate-200 bg-white transition focus-within:border-[color:var(--ab-acento)] focus-within:ring-4 focus-within:ring-[rgb(var(--ab-acento-rgb)_/_0.15)] sm:flex sm:h-11 sm:items-stretch sm:self-start xl:self-auto">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                <Hammer className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Detalle de servicios
              </h2>
              <span className="text-xs font-semibold text-slate-400">
                {periodoTexto}{filtros.tecnico ? ` · ${filtros.tecnico}` : ''}
              </span>
              {cargando && (
                <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[color:var(--ab-acento)]">
                  <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-[rgb(var(--ab-acento-rgb)_/_0.25)] border-t-[color:var(--ab-acento)]" /> Actualizando…
                </span>
              )}
            </div>
            {hayFiltros && (
                  ...
                  <table className="w-full min-w-[1080px] text-[13px]">
                      ...
                      {visibles.map((s) => (
                        <tr key={s.id} className="align-top transition-colors hover:bg-slate-50/70">
                          <td className="px-5 py-3">
                            <p className="font-mono text-[13px] font-bold text-[color:var(--ab-acento)]">{s.codigo_nota || '—'}</p>
                            <p className="mt-0.5 whitespace-nowrap text-xs text-slate-400">{fechaCorta(s.fecha)}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="max-w-[190px] truncate font-semibold text-slate-900">{s.cliente}</p>
                            <p className="mt-0.5 text-xs text-slate-400">{s.telefono || 'Sin teléfono'} · por {s.vendedor?.name || '—'}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="max-w-[260px] truncate font-medium text-slate-800">{s.equipo}</p>
                            <p className="mt-0.5 max-w-[260px] truncate text-xs text-slate-500" title={s.trabajos.map((t) => t.descripcion).join(' · ')}>
                              {s.trabajos[0]?.descripcion ?? '—'}
                              {s.trabajos.length > 1 && <span className="text-slate-400"> y {s.trabajos.length - 1} más</span>}
                            </p>
                          </td>
                              ...
                              {s.pendiente ? (
                                ...
                                <span className="inline-flex items-center justify-end gap-1 whitespace-nowrap">
                                  ...
                                  <button type="button" onClick={() => setCargandoCosto(s)} aria-label={`Corregir el costo de ${s.codigo_nota || 'este servicio'}`}
                                    title="Corregir el costo" className="grid h-6 w-6 place-items-center rounded-md text-slate-300 transition-colors hover:bg-slate-100 hover:text-slate-600">
                          ...
                          <td className="whitespace-nowrap px-4 py-3 text-right font-bold tabular-nums text-slate-900">{bsFmt(s.cobrado)}</td>
                          {conCostos && (
                            <td className="whitespace-nowrap px-4 py-3 text-right tabular-nums">
                              {s.pendiente ? (
                                ...
                                <span className="inline-flex flex-col items-end">
                                  ...
                                  <span className="text-[11px] text-slate-400">Bajo el costo</span>
                                </span>
                              ) : (
                    ...
                    <tfoot>
                      <tr className="border-t border-slate-200 bg-slate-50/70 text-[13px]">
                        ...
                        {conCostos && <td className="px-4 py-3 text-right font-semibold tabular-nums text-slate-600">{bsFmt(totalCosto)}</td>}
                  ...
                  {visibles.map((s) => (
                    <li key={s.id} className="space-y-3 px-4 py-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate font-semibold text-slate-900">{s.cliente}</p>
                          <p className="font-mono text-xs font-bold text-[color:var(--ab-acento)]">
                            {s.codigo_nota || '—'} <span className="font-sans font-normal text-slate-400">· {fechaCorta(s.fecha)}</span>
                          </p>
                        </div>
                        <Tecnico nombre={s.tecnico} />
                      </div>
                      <p className="text-sm text-slate-600">
                        <span className="font-semibold text-slate-900">{s.equipo}</span>
                        {s.trabajos[0] && ` · ${s.trabajos[0].descripcion}`}
                        {s.trabajos.length > 1 && <span className="text-slate-400"> y {s.trabajos.length - 1} más</span>}
                      </p>
                      <div className={`grid gap-2 rounded-xl bg-slate-50 p-3 text-center ${conCostos ? 'grid-cols-3' : 'grid-cols-1'}`}>
                        {conCostos && <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Costo</p><p className="text-sm font-semibold tabular-nums text-slate-600">{s.pendiente ? '—' : bsFmt(s.costo)}</p></div>}
                        <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Cobrado</p><p className="text-sm font-bold tabular-nums text-slate-900">{bsFmt(s.cobrado)}</p></div>
                        {conCostos && <div><p className="text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">Ganancia</p><p className={`text-sm font-bold tabular-nums ${s.pendiente ? 'text-slate-400' : s.ganancia < 0 ? 'text-rose-600' : 'text-emerald-700'}`}>{s.pendiente ? 'Pendiente' : conSigno(s.ganancia)}</p></div>}
                      </div>
