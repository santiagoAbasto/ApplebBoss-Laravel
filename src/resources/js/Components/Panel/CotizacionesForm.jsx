...
const labelCls = 'block text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400';
...
function Negritas({ texto: t }) {
  return t.split(/(\*\*[^*]+\*\*)/g).map((parte, i) => (/^\*\*[^*]+\*\*$/.test(parte)
    ? <strong key={i} className="font-bold text-slate-900">{parte.slice(2, -2)}</strong>
...
function NotasPrevia({ texto: t }) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4 text-sm leading-relaxed text-slate-700">
      {bloques.map((b, i) => {
        ...
        if (/^\*\*[^*]+\*\*$/.test(unido)) {
          return <p key={i} className="font-bold text-[#011446]">{unido.slice(2, -2)}</p>;
        }
...
function Importe({ label, valor, className = 'text-slate-900', fuerte = false }) {
  return (
    <div className="flex justify-between gap-3">
      <dt className={fuerte ? 'font-semibold text-slate-700' : 'text-slate-500'}>{label}</dt>
...
export default function CotizacionesForm({
  ...
  return (
    <Layout title="Nueva cotización">
      <Head title="Nueva cotización | Apple Boss" />
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Arma la propuesta para el cliente. Al guardar se genera el PDF con el total con y sin factura.</p>
          </div>
        </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                      ...
                      {sugerencias.map((c) => (
                        <li key={c.id}>
                          <button type="button" className="block w-full px-4 py-2.5 text-left hover:bg-slate-50"
                            ...
                            <span className="text-xs text-slate-500">{[c.telefono, c.correo].filter(Boolean).join(' · ')}</span>
                          </button>
                        </li>
                      ))}
                    </ul>
                ...
                <Field label="WhatsApp" error={errores.telefono_completo}
                  ...
                  <PhoneInput
                    ...
                    className={`flex h-11 items-center gap-2 rounded-xl border bg-white px-3 transition focus-within:border-[color:var(--ab-acento)] focus-within:ring-4 focus-within:ring-[rgb(var(--ab-acento-rgb)_/_0.15)] ${errores.telefono_completo ? 'border-rose-300' : 'border-slate-200'}`}
                    numberInputProps={{ className: 'h-full min-w-0 flex-1 border-0 bg-transparent p-0 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-0' }}
                  />
                </Field>
            ...
            <StepCard step={2} title="Productos"
              ...
              <div className="relative mt-3">
                ...
                {mostrarResultados && busqueda.trim() && (
                  <ul className="absolute left-0 right-0 top-full z-30 mt-2 max-h-80 overflow-auto rounded-xl border border-slate-200 bg-white shadow-xl">
                    {resultados.length === 0 ? (
                      <li className="px-4 py-3 text-sm text-slate-500">Nada coincide en {etiqueta.toLowerCase()} disponibles.</li>
                    ) : resultados.map((p) => {
                        ...
                        <li key={p.id}>
                          <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => agregarDelInventario(p)}
                            ...
                            <span className="min-w-0">
                              <span className="block truncate font-semibold text-slate-900">{p.modelo || p.nombre}</span>
                              {detalle && <span className="block truncate text-xs text-slate-500">{detalle}</span>}
                            </span>
              ...
              <div className="mt-2.5 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
                ...
                <button type="button" onClick={agregarExterno}
                  className="inline-flex items-center gap-1.5 font-semibold text-[color:var(--ab-acento)] transition-colors hover:text-[#011446]">
              ...
              <div className="mt-5">
                {lineas.length === 0 ? (
                  <div className={`rounded-xl border border-dashed px-4 py-8 text-center text-sm ${errores.items ? 'border-rose-300 bg-rose-50/40 font-semibold text-rose-600' : 'border-slate-200 bg-slate-50/60 text-slate-500'}`}>
                  ...
                  <ul className="space-y-3">
                    {lineas.map((i, idx) => {
                      ...
                      return (
                        <li key={i.key} className={`rounded-xl border p-4 transition-colors ${error ? 'border-rose-300 bg-rose-50/30' : 'border-slate-200 bg-white'}`}>
                          <div className="flex items-start gap-3">
                            <span className="mt-1.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-slate-100 text-xs font-bold text-slate-500">{idx + 1}</span>
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
                <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                  <FileText className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Resumen
                </h2>
              ...
              <div className="space-y-4 p-5">
                ...
                <p className="flex items-start justify-center gap-1.5 text-center text-xs text-slate-400">
                  {correoOk ? (
