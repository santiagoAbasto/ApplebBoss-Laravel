...
const TERMINOS = 'La reserva se descuenta del precio final al concretar la venta. El producto queda separado mientras la reserva permanezca activa. En caso de cancelación, vencimiento o cambio de producto, Apple Boss debe autorizar el ajuste antes de liberar el inventario.';

const TIPOS_PRODUCTO = [
  { value: 'celular', label: 'Celular', icon: Smartphone },
  { value: 'computadora', label: 'Computadora', icon: Laptop },
  { value: 'producto_apple', label: 'Producto Apple', icon: Tablet },
...
export default function ReservasForm({ Layout, prefijo = 'admin', guia = null }) {
  ...
  return (
    <Layout title="Nueva reserva">
      <Head title="Nueva reserva | Apple Boss" />
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Separa productos con un abono. Al guardar se abre la nota para el cliente.</p>
          </div>
        </div>
          ...
          <div className="min-w-0 space-y-5">
            ...
            <StepCard step={2} title="Productos a separar" subtitle="Elige el tipo y busca por código, IMEI, serie o nombre.">
              ...
              <div className="relative mt-4">
                ...
                />
                {mostrarProductos && (
                  <ul className="absolute left-0 right-0 top-full z-30 mt-2 max-h-72 overflow-auto rounded-xl border border-slate-200 bg-white shadow-xl">
                      ...
                      <li key={i}>
                        <button type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => seleccionarProducto(p)}
                          ...
                          <span className="min-w-0">
                            <span className="block truncate font-semibold text-slate-900">{p.nombre || p.modelo}</span>
                            <span className="block truncate text-xs text-slate-500">{etiquetaTipo(p.tipo)} · {p.codigo || p.imei_1 || p.numero_serie}</span>
                          </span>
                          <span className="shrink-0 text-sm font-bold tabular-nums text-slate-900">{bsFmt(p.precio_venta)}</span>
                        </button>
                      </li>
                    ))}
                  </ul>
              ...
              {productoActual && (
                <div className="mt-4 rounded-xl border border-[rgb(var(--ab-acento-rgb)_/_0.25)] bg-[rgb(var(--ab-acento-rgb)_/_0.05)] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[color:var(--ab-acento)]">Producto elegido</p>
                      <p className="mt-1 truncate text-base font-bold text-slate-900">{productoActual.modelo || productoActual.nombre}</p>
                    ...
                    <p className="text-xl font-extrabold text-slate-900">{bsFmt(productoActual.precio_venta)}</p>
                  </div>
              ...
              <div className="mt-5">
                <p className="mb-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Separados · {items.length}</p>
                {items.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {items.map((item, i) => (
                      <li key={`${item.tipo}-${item.producto_id}`} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3">
                        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-slate-100 text-xs font-bold text-slate-500">{i + 1}</span>
                        ...
                        <p className="shrink-0 text-sm font-bold tabular-nums text-slate-900">{bsFmt(item.subtotal)}</p>
            ...
            <StepCard step={3} title="Abono y condiciones" subtitle="Cuánto deja el cliente hoy. El resto se cobra al vender.">
              <div className="grid gap-4 md:grid-cols-[minmax(0,260px)_1fr] md:items-start">
                ...
                {sugerenciasAbono.length > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-600">Montos sugeridos</p>
                    <div className="flex flex-wrap gap-2">
                      {sugerenciasAbono.map((m, i) => (
                        <button key={i} type="button" onClick={() => setData({ ...data, monto_reserva: String(m) })}
                          className={`h-11 rounded-xl border px-4 text-sm font-semibold transition-colors ${abono === m ? 'border-[#011446] bg-[#011446] text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:text-slate-900'}`}>
                          {[20, 30, 50][i]}% · {bsFmt(m)}
                        </button>
                      ))}
                    </div>
              ...
              <div className="mt-5">
                <div className="mb-1.5 flex items-center justify-between gap-3">
                  <span className="text-xs font-semibold uppercase tracking-wide text-slate-600">Condiciones que se imprimen en la nota</span>
                  {data.terminos_condiciones !== TERMINOS && (
                    <button type="button" onClick={() => setData({ ...data, terminos_condiciones: TERMINOS })}
                      className="inline-flex items-center gap-1 text-xs font-semibold text-[color:var(--ab-acento)] hover:text-[#011446]">
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
                <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                  <CalendarCheck className="h-[18px] w-[18px] text-[color:var(--ab-acento)]" /> Resumen de la reserva
                </h2>
              ...
              <div className="space-y-4 p-5">
                  ...
                  <ul className="max-h-48 space-y-2 overflow-y-auto">
                    {items.map((item) => (
                      <li key={`r-${item.tipo}-${item.producto_id}`} className="flex items-baseline justify-between gap-3 text-sm">
                        <span className="min-w-0 truncate text-slate-600">{item.nombre}</span>
                        <span className="shrink-0 font-semibold tabular-nums text-slate-900">{bsFmt(item.subtotal)}</span>
                      </li>
                    ))}
                  </ul>
                )}
                ...
                <dl className="space-y-1.5 border-t border-slate-100 pt-4 text-sm">
                  <div className="flex justify-between gap-3"><dt className="text-slate-500">Total reservado</dt><dd className="font-semibold tabular-nums text-slate-900">{bsFmt(subtotal)}</dd></div>
                  <div className="flex justify-between gap-3"><dt className="text-slate-500">Abono de hoy</dt><dd className="font-semibold tabular-nums text-emerald-700">−{bsFmt(abono)}</dd></div>
                </dl>
                ...
                <p className="text-center text-xs text-slate-400">Los productos quedan separados y se abre la nota.</p>
              </div>
            </section>
          </aside>
        </div>
