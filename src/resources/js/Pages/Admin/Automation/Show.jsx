        ...
        <AdminLayout>
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-sky-50 to-blue-100 p-4 sm:p-6 lg:p-8">
                <div className="mx-auto max-w-7xl space-y-8">
                    <header className="rounded-[2rem] bg-white/90 p-6 shadow-xl ring-1 ring-slate-200/70 sm:p-8">
                    ...
                    <Section title="Crecimiento Mensual" subtitle="Comparación contra el período mensual anterior equivalente.">
                        ...
                        >
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <p className="text-sm font-medium text-slate-600">Variación mensual</p>
...
function MetricCard({ title, value, color }) {
    ...
    return (
        <div className="rounded-[2rem] bg-white p-6 shadow-lg ring-1 ring-slate-200/70">
            <div className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${colors[color]}`}>
                {title}
            </div>
            <div className="mt-4 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{value}</div>
        </div>
    );
}
...
function Section({ title, subtitle, children }) {
    return (
        <section className="rounded-[2rem] bg-white p-6 shadow-xl ring-1 ring-slate-200/70 sm:p-8">
            <div className="mb-6">
...
function AnalysisBlock({ data }) {
    if (!data || typeof data !== "object") {
        return <p className="text-sm text-slate-500">No hay información analítica disponible para esta sección.</p>;
    }
...
function renderValue(key, value) {
    if (Array.isArray(value)) {
        if (value.length === 0) {
            return <span className="text-slate-400">Sin registro suficiente</span>;
        }
            ...
            return (
                <div className="space-y-2">
                    {value.map((item, index) => (
                        <div key={`${key}-${index}`} className="rounded-xl bg-white p-3 ring-1 ring-slate-200">
            ...
            <div className="grid gap-2">
                {Object.entries(value).map(([childKey, childValue]) => (
                    <div key={childKey} className="flex flex-wrap gap-2">
                        <span className="font-medium text-slate-800">{labelFor(childKey)}:</span>
...
function PriorityBadge({ value }) {
    ...
    return (
        <span className={`inline-flex w-fit rounded-full px-4 py-2 text-xs font-semibold ${tones[value] || "bg-slate-100 text-slate-700"}`}>
            {emptyText(value)}
        </span>
    );
}
