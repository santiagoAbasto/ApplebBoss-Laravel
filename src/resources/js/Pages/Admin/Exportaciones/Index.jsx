...
function TarjetaExportar({ icon: Icon, titulo, detalle, href, vacia = false, nuevaPestana = true }) {
  const contenido = (
    <>
      <div className="flex items-start gap-3">
        <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${vacia ? 'bg-slate-100 text-slate-400' : 'bg-[#011446]/[0.07] text-[#011446]'}`}>
          <Icon className="h-5 w-5" />
        </span>
        <div className="min-w-0">
          <p className="text-[15px] font-bold text-slate-900">{titulo}</p>
          <p className="mt-0.5 text-[13px] leading-relaxed text-slate-500">{detalle}</p>
        </div>
      </div>
      <span className={`mt-3 inline-flex items-center gap-1.5 text-xs font-bold ${vacia ? 'text-slate-400' : 'text-[#585E9F]'}`}>
  ...
  const clase = `block rounded-2xl border p-4 text-left transition-shadow ${vacia
    ? 'cursor-not-allowed border-slate-200/80 bg-slate-50/60'
    : 'border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)] hover:shadow-[0_8px_24px_rgba(15,23,42,0.08)]'}`;
  ...
  return (
    <AdminLayout>
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        {sinDatosDeContacto && (
          <Aviso tono="amber" icon={MapPin} accion="Ir a Ubicaciones"
            onAccion={() => window.location.assign(route('admin.locations.index'))}>
            <span className="font-bold">Al pie de los PDF le faltan tus datos.</span>{' '}
            {!tienda.direccion && !tienda.telefono
              ? 'No hay dirección ni teléfono cargados en Ubicaciones.'
              : !tienda.direccion ? 'Falta la dirección del local en Ubicaciones.' : 'Falta el teléfono del local en Ubicaciones.'}
          </Aviso>
        )}
        ...
        <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
            <Boxes className="h-[18px] w-[18px] text-[#585E9F]" /> Por inventario
          </h2>
          <p className="mt-0.5 text-[13px] text-slate-500">Todo lo que está disponible hoy, en un solo listado.</p>
        ...
        <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
            <Tags className="h-[18px] w-[18px] text-[#585E9F]" /> Por tipo de producto general
            <span className="text-sm font-semibold text-slate-400">{subtipos.length}</span>
          </h2>
