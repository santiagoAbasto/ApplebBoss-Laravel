...
import { Field, Input, PageHeader, Segmented, StepCard, Switch, Toast, buttonCls, useToast } from '@/Components/Admin/ui';
    ...
    <AdminLayout>
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <AdminGuide id="exportador" title="¿Cómo busco lo que necesito?" steps={[
          'Elige en qué inventario buscar: celulares, computadoras, productos Apple o productos generales.',
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="min-w-0 space-y-5">
            <StepCard step="1" title="¿Dónde busco?" subtitle="Cada inventario se busca por su propio campo.">
              ...
              <label className="mt-4 flex cursor-pointer items-center justify-between gap-4 rounded-xl bg-slate-50 px-3.5 py-3">
                <span className="min-w-0">
                  <span className="block text-[13px] font-bold text-slate-900">Solo lo disponible</span>
            ...
            <StepCard step="2" title="¿Qué busco?" subtitle="Las palabras del nombre o del modelo, como las tienes cargadas.">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className="text-xs font-semibold text-slate-500">Ejemplos:</span>
                {(EJEMPLOS[inventario] ?? []).map((ej) => (
                  <button key={ej} type="button" onClick={() => setNombre(ej)}
                    className="rounded-lg border border-dashed border-slate-300 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:border-[#585E9F] hover:text-[#585E9F]">
                    {ej}
                  </button>
                ))}
              </div>
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <h2 className="text-base font-bold text-slate-900">Cómo buscar bien</h2>
              <p className="mt-0.5 text-[13px] text-slate-500">La búsqueda es simple a propósito: encuentra por palabras sueltas.</p>
          ...
          <aside className="space-y-5 xl:sticky xl:top-24">
            <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <h2 className="text-base font-bold text-slate-900">Lo que va a salir</h2>
                ...
                <ul className="mt-4 divide-y divide-slate-100 rounded-xl border border-slate-100">
                  {cuenta.muestra.map((p, i) => (
                    <li key={`${p.nombre}-${i}`} className="flex items-center justify-between gap-3 px-3.5 py-2.5">
                      <span className="min-w-0 truncate text-[13px] text-slate-700">{p.nombre}</span>
                      <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${p.estado === 'disponible' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
              ...
              <p className="mt-2 text-center text-[11px] text-slate-400">Se abre en otra pestaña.</p>
            </section>
