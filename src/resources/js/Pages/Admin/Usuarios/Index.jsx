...
import {
  KeyRound, Pencil, Plus, Search, Shield, ShieldCheck, Trash2, TriangleAlert, UserPlus, Users, X,
} from 'lucide-react';
...
function FormularioUsuario({ usuario, roles, onCerrar }) {
    ...
    >
      <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_260px]">
        ...
        <div className="grid content-start gap-3 md:sticky md:top-0">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Con ese rol entra a</p>
          <div className="rounded-xl border border-slate-200 px-3.5 py-3">
            <p className="text-[13px] font-bold text-slate-900">{rolElegido?.nombre ?? 'Sin rol'}</p>
            {rolElegido?.descripcion && (
              <p className="mt-0.5 text-xs leading-relaxed text-slate-500">{rolElegido.descripcion}</p>
            )}
      ...
      <div className="grid gap-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Nombre del rol" error={errors.nombre} hint="Es el que se elige al crear un usuario.">
            <Input value={data.nombre} maxLength={60} autoFocus placeholder="Encargado de tienda"
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">A qué parte del panel entra</p>
        ...
        {editando && !rol.del_sistema && (
          <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl bg-slate-50 px-3.5 py-3">
            <span className="min-w-0">
              <span className="block text-[13px] font-bold text-slate-900">Rol activo</span>
            ...
            <input type="checkbox" checked={data.activo} onChange={(e) => setData('activo', e.target.checked)}
              className="h-5 w-5 rounded border-slate-300 text-[#585E9F]" />
          </label>
        )}
...
export default function UsuariosIndex({ usuarios = [], roles = [], permisos = [], filtros = {}, yo, resumen = {} }) {
  ...
  return (
    <AdminLayout>
      ...
      {formUsuario && (
        <FormularioUsuario usuario={formUsuario.usuario} roles={roles} onCerrar={() => setFormUsuario(null)} />
      )}
      {formRol && (
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        ]} tip="El administrador entra a todo, siempre, y nunca te puedes quitar el rol a ti mismo ni borrar al último administrador: es la forma de que nadie quede afuera de la configuración de la tienda." />

        {(resumen.admins ?? 0) === 1 && (
          <Aviso tono="lila" icon={ShieldCheck}>
            <span className="font-bold">Hay un solo administrador.</span>{' '}
            Si pierdes esa contraseña, nadie puede entrar a configurar la tienda. Conviene tener un segundo.
          </Aviso>
        )}
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
          <div className="min-w-0 space-y-5">
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                      <IconoUsuarios className="h-[18px] w-[18px] text-[#585E9F]" /> Usuarios
                      <span className="text-sm font-semibold text-slate-400">{usuarios.length}</span>
                    </h2>
                    <p className="mt-0.5 text-[13px] text-slate-500">Los administradores van primero.</p>
                  </div>
              ...
              {usuarios.length === 0 ? (
                <div className="px-5 py-12 text-center">
                  ...
                  <p className="mt-3 text-sm font-bold text-slate-800">Ningún usuario coincide</p>
                  <p className="mx-auto mt-1 max-w-sm text-[13px] text-slate-500">Prueba con otro filtro o con parte del correo.</p>
                </div>
              ) : (
                <ul className="divide-y divide-slate-100">
                  {usuarios.map((u) => (
                    <li key={u.id} className="flex flex-col gap-3 px-5 py-4 lg:flex-row lg:items-center">
                      <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-full text-[13px] font-extrabold ${u.rol === 'admin' ? 'bg-[#011446] text-white' : 'bg-[#585E9F]/12 text-[#585E9F]'}`}>
                        {iniciales(u.name)}
                      </span>
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="flex flex-wrap items-center gap-2 text-[15px] font-bold text-slate-900">
                          {u.name}
                          {u.id === yo && <span className="rounded-full bg-[#C6CB36]/25 px-2 py-0.5 text-[10px] font-bold text-[#4a4f10]">TÚ</span>}
                        </p>
                        <p className="mt-0.5 truncate text-[13px] text-slate-500">{u.email}</p>
                        <p className="mt-1 text-xs text-slate-400">Desde {fmtDate(u.created_at)}</p>
                      </div>

                      <div className="lg:w-[150px] lg:shrink-0">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold ${u.rol === 'admin' ? 'bg-[#011446]/[0.08] text-[#011446]' : 'bg-slate-100 text-slate-600'}`}>
                      ...
                      <div className="flex flex-wrap items-center gap-2 lg:shrink-0">
                        ...
                        <button type="button" onClick={() => setBorrarUsuario(u)} disabled={u.id === yo}
                          ...
                          className="grid h-9 w-9 place-items-center rounded-xl text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-slate-400">
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <h2 className="text-base font-bold text-slate-900">Cómo dar acceso sin perder el control</h2>
              <p className="mt-0.5 text-[13px] text-slate-500">El panel tiene los costos, los precios y los datos de tus clientes.</p>
              <Consejos
                consejos={CONSEJOS_USUARIOS}
                cierre={(
                  <>
                    <span className="font-bold text-slate-800">Si alguien olvidó su contraseña:</span> edítalo acá y
          ...
          <aside className="space-y-5 xl:sticky xl:top-24">
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
                <div className="min-w-0">
                  <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                    <Shield className="h-[18px] w-[18px] text-[#585E9F]" /> Roles
                    <span className="text-sm font-semibold text-slate-400">{roles.length}</span>
                  </h2>
                  <p className="mt-0.5 text-[13px] text-slate-500">Cada uno abre su parte del panel.</p>
                </div>
              ...
              <ul className="divide-y divide-slate-100">
                {roles.map((r) => (
                  <li key={r.id} className={`px-5 py-4 ${r.activo ? '' : 'bg-slate-50/60'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="flex flex-wrap items-center gap-2 text-[15px] font-bold text-slate-900">
                          {r.nombre}
                          {r.del_sistema && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-500">DEL SISTEMA</span>}
                      ...
                      <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        <button type="button" onClick={() => setBorrarRol(r)} disabled={r.del_sistema || r.usuarios > 0}
                          ...
                          className="grid h-8 w-8 place-items-center rounded-xl text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600 disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-slate-400">
