...
import {
  ...
  checkCls, motivoEnListado, paginar, useSeleccion,
...
export default function Index({ computadoras = [], enTienda = {}, conHistorial = [] }) {
  useAutoRefresh(['computadoras', 'enTienda', 'conHistorial']);
  ...
  return (
    <AdminLayout>
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        <PageHeader
          title="Computadoras"
          subtitle="Inventario de Mac y otras computadoras: las disponibles van primero. La procedencia es interna; el número de serie sí se muestra en la tienda."
          actions={(
            <>
        ...
        {sinCondicion > 0 && !(estado === 'disponible' && condicion === 'sin') && (
          <Aviso onAccion={() => { setEstado('disponible'); setCondicion('sin'); }}>
            ...
            (nuevo o seminuevo). Selecciónalas y márcalas de una vez: así se publican en la tienda sin pasos extra.
          </Aviso>
        )}
        ...
        <section ref={tablaRef} className="scroll-mt-24 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
              <Laptop className="h-[18px] w-[18px] text-[#585E9F]" /> Listado de computadoras
            </h2>
                    ...
                    {visibles.map((c) => {
                      ...
                      return (
                        <tr key={c.id} className={`transition-colors ${marcado ? 'bg-[#585E9F]/[0.06]' : 'hover:bg-slate-50/70'}`}>
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <Link href={route('admin.computadoras.edit', c.id)} className="block max-w-[280px] truncate font-semibold text-slate-900 hover:text-[#585E9F]">
                                  {c.nombre}
                                </Link>
                                <div className="mt-0.5 flex flex-wrap items-center gap-1.5">
                                  <span className="truncate text-xs text-slate-500">{c.detalle || 'Sin chip ni memoria'}</span>
                          ...
                          <td className="px-3 py-3 font-mono text-xs text-slate-800">{c.numero_serie || '—'}</td>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              ...
                              <Link href={route('admin.computadoras.edit', c.id)} className="mt-1.5 block truncate text-[15px] font-bold text-slate-900">{c.nombre}</Link>
                              <p className="truncate text-xs text-slate-500">{c.detalle || 'Sin chip ni memoria'}</p>
                            </div>
                          ...
                          <div className="mt-2.5 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs text-slate-500">
                            <span className="font-mono text-slate-700">{c.numero_serie}</span>
