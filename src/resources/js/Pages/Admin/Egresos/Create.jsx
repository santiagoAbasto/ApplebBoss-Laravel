...
const TIPOS = [
  { value: 'servicio_basico', label: 'Servicio básico', icon: Zap, ayuda: 'Luz, agua, internet, alquiler y otros servicios del local.', ejemplo: 'Ej.: Luz de la tienda' },
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Registra un gasto del negocio con la fecha de hoy. Se resta de la utilidad disponible del resumen.</p>
          </div>
        </div>
        ...
        <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="min-w-0 space-y-5">
            ...
            <StepCard step={1} title="Tipo de gasto" subtitle="Elige a qué corresponde: así el resumen separa bien los gastos.">
              ...
              <p className="mt-3 flex items-start gap-2 rounded-xl bg-slate-50 px-3 py-2.5 text-[13px] text-slate-600">
                <Info className="mt-0.5 h-4 w-4 shrink-0 text-[#585E9F]" /> {tipo.ayuda}
              </p>
            </StepCard>
              ...
              <div className={`mt-4 grid gap-4 ${esCuota ? 'md:grid-cols-[minmax(0,1fr)_240px]' : ''}`}>
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {FRECUENCIAS.map((f) => (
                      <button key={f} type="button" onClick={() => cambiar('frecuencia', data.frecuencia === f ? '' : f)} aria-pressed={data.frecuencia === f}
                        className={`rounded-lg px-2.5 py-1 text-xs font-bold transition-colors ${data.frecuencia === f ? 'bg-[#011446] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                        {f}
                      </button>
                    ))}
                  </div>
                ...
                {esCuota && (
                  <Field label="Cuotas pendientes" error={errores.cuotas_pendientes} hint="Cuántas cuotas faltan después de esta.">
                    <div className="flex h-11 items-stretch overflow-hidden rounded-xl border border-slate-200 bg-white transition focus-within:border-[#585E9F] focus-within:ring-4 focus-within:ring-[#585E9F]/15">
                      <button type="button" onClick={() => ajustarCuotas(-1)} aria-label="Una cuota menos"
                        className="grid w-11 shrink-0 place-items-center text-slate-500 transition-colors hover:bg-slate-50 hover:text-slate-900">
                      ...
                      <input type="number" min="0" max={MAX_CUOTAS} step="1" inputMode="numeric" placeholder="0" aria-label="Cuotas pendientes"
                        ...
                        className="w-full min-w-0 border-x border-y-0 border-slate-200 bg-transparent px-2 text-center text-sm font-semibold tabular-nums text-slate-900 focus:outline-none focus:ring-0" />
            ...
            <section className="rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="border-b border-slate-100 px-5 py-4">
                <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                  <Wallet className="h-[18px] w-[18px] text-[#585E9F]" /> Resumen del egreso
                </h2>
              </div>

              <div className="space-y-4 p-5">
                <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3">
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#585E9F]/10 text-[#585E9F]">
                    <TipoIcon className="h-5 w-5" />
                  </span>
                  <div className="min-w-0">
                    <p className="text-xs text-slate-500">Tipo de gasto</p>
                    <p className="truncate font-bold text-slate-900">{tipo.label}</p>
                  </div>
                </div>
                ...
                <p className="text-center text-xs text-slate-400">Al guardar vuelves al listado de egresos.</p>
              </div>
            </section>
          </aside>
        </div>
