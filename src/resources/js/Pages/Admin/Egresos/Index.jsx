...
function Cuotas({ e }) {
  if (e.tipo_gasto !== 'cuota_bancaria') return <span className="text-slate-300">—</span>;
  const n = e.cuotas_pendientes;
  if (n === null || n === undefined || n === '') return <span className="text-slate-400">Sin dato</span>;
  return <span className="font-semibold text-slate-700">{n} {Number(n) === 1 ? 'pendiente' : 'pendientes'}</span>;
}
      ...
      <div className="ab-reset mx-auto max-w-[1400px] space-y-5">
        ...
        <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="mt-3 flex flex-col gap-3 border-t border-slate-100 pt-3 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-slate-200 bg-white transition focus-within:border-[#585E9F] focus-within:ring-4 focus-within:ring-[#585E9F]/15 sm:flex sm:h-11 sm:items-stretch sm:self-start xl:self-auto">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              <h2 className="flex items-center gap-2 text-base font-bold text-slate-900">
                <Wallet className="h-[18px] w-[18px] text-[#585E9F]" /> Detalle de egresos
              </h2>
              <span className="text-xs font-semibold text-slate-400">{periodoTexto}</span>
              {cargando && (
                ...
                <div className="hidden overflow-x-auto md:block">
                  <table className="w-full min-w-[980px] text-[13px]">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {visibles.map((e) => (
                        <tr key={e.id} className="align-top transition-colors hover:bg-slate-50/70">
                          <td className="px-5 py-3">
                            <p className="whitespace-nowrap font-semibold text-slate-800">{fechaCorta(e.created_at)}</p>
                            <p className="mt-0.5 text-xs text-slate-400">{hora(e.created_at)}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="max-w-[280px] truncate font-semibold text-slate-900">{e.concepto}</p>
                            {e.comentario && <p className="mt-0.5 max-w-[280px] truncate text-xs text-slate-500" title={e.comentario}>{e.comentario}</p>}
                          </td>
                          <td className="px-4 py-3"><TipoBadge tipo={e.tipo_gasto} /></td>
                          <td className="px-4 py-3 text-slate-600">{mayuscula(e.frecuencia) || <span className="text-slate-300">—</span>}</td>
                          <td className="px-4 py-3"><Cuotas e={e} /></td>
                          <td className="px-4 py-3 text-slate-600">{e.user?.name || '—'}</td>
                          <td className="px-5 py-3 text-right font-bold tabular-nums text-slate-900">{bsFmt(e.monto)}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                    ...
                    <li key={e.id} className="space-y-2.5 px-4 py-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate font-semibold text-slate-900">{e.concepto}</p>
                          <p className="text-xs text-slate-400">{fechaCorta(e.created_at)} · {hora(e.created_at)} · {e.user?.name || '—'}</p>
                        </div>
                        <p className="shrink-0 text-base font-extrabold tabular-nums text-slate-900">{bsFmt(e.monto)}</p>
                      </div>
                      ...
                      {e.comentario && <p className="text-xs text-slate-500">{e.comentario}</p>}
                    </li>
                  ))}
                </ul>
